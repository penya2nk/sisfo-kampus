...
$namaFile = "$prodi[Nama]_".$_GET[tahun]."_Jadwal_Skripsi.xls";
...
<th width='120px' align='center'>ANGKA</th>
<th width='120px' align='center'>HURUF</th>


</tr>";	
	
$sq = mysqli_query($koneksi, "SELECT * from vw_jadwal_skripsi_ujian where TahunID='".strfilter($_GET[tahun])."' and ProdiID='".strfilter($_GET[prodi])."' order by TglUjianSkripsi,JamMulaiProSkripsi asc");
...
$x1     = mysqli_fetch_array(mysqli_query($koneksi, "SELECT Login,Nama,Gelar FROM dosen where Login='$r[PengujiSkripsi1]'"));
$x2     = mysqli_fetch_array(mysqli_query($koneksi, "SELECT Login,Nama,Gelar FROM dosen where Login='$r[PengujiSkripsi2]'"));
$x3     = mysqli_fetch_array(mysqli_query($koneksi, "SELECT Login,Nama,Gelar FROM dosen where Login='$r[PengujiSkripsi3]'"));
...
echo"<tr>
	...
	<td align='right' width='120px'>" .$dayList[$day].",".$r[TglUjianSkripsi]. " <br> " .$r[JamMulaiUjianSkripsi]. "-" .$r[JamSelesaiUjianSkripsi]. " <br> ".$d[Nama]. " <br></td> 
	<td align=left >1. $penguji1, $x1[Gelar]  <br> 2. $penguji2, $x2[Gelar]<br> 3. $penguji3, $x3[Gelar]<br></td>  	
	<td align='right' width='120px'>" .substr($r[NilaiPengujiSkripsi1],0,2). " <br> " .substr($r[NilaiPengujiSkripsi2],0,2). " <br> " .substr($r[NilaiPengujiSkripsi3],0,2). " <br></td>
