...
<input type="hidden" name='ndelox' value='students/ppmi-angketmhsvisi'>
...
<tr><td colspan=2>
<a href='index.php?ndelox=students/ppmi-angketmhsvisi&act=tambahdata&JadwalID=$r[JadwalID]&tahun=$_SESSION[tahun_akademik]&MKID=$r[MKID]&DosenID=$r[DosenID]&prodi=$r[ProdiID]'> | Klik Link Pengisian Angket</a>
| <a href='index.php?ndelox=students/ppmi-angketmhsvisi&act=viewangket&JadwalID=$r[JadwalID]&tahun=$_SESSION[tahun_akademik]&MKID=$r[MKID]&DosenID=$r[DosenID]&prodi=$r[ProdiID]'> Lihat Angket</a>
</td></tr>
...
$prdmhs = mysqli_fetch_array(mysqli_query($koneksi, "SELECT MhswID,ProdiID FROM mhsw WHERE MhswID='".$_SESSION['_Login']."'"));
echo"
<div class='card'>
<div class='card-header'>
<div class='table-responsive'>		
...
<input type='hidden' name='prodi' value='$prdmhs[ProdiID]'>	
...
<a href='index.php?ndelox=students/ppmi-angketmhsvisi&tahun=$_GET[tahun]&prodi=$_GET[prodi]'><button type='button' class='btn btn-default pull-right'>Back</button></a> 
...
$JumData 	= mysqli_num_rows(mysqli_query($koneksi, "SELECT * from t_ppmiangketvisi"));	
for($i = 1; $i <= $JumData; $i++){
	$PertanyaanID  	= strfilter($_POST['PertanyaanID'.$i]);
    $Jawaban  		= strfilter($_POST['Jawaban'.$i]);	
		if (!empty($PertanyaanID)){     
			$sqlcek = mysqli_query($koneksi, "SELECT * FROM t_ppmiangketvisi WHERE MhswID='$_SESSION[_Login]' and PertanyaanID='$PertanyaanID'"); // and JadwalID='$JadwalID'
			if (mysqli_num_rows($sqlcek)>0){
				...
				window.location = 'index.php?ndelox=students/ppmi-angketmhsvisi&JadwalID=$_POST[JadwalID]&tahun=$_POST[tahun]&prodi=$_POST[prodi]&gagal'</script>";
				exit;
			}
	...
	if ($query){
		echo "<script>document.location='index.php?ndelox=students/ppmi-angketmhsvisi&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&sukses';</script>";                       
	}else{
		echo "<script>document.location='index.php?ndelox=students/ppmi-angketmhsvisi&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&gagal';</script>";
	}
...
$angket = mysqli_fetch_array(mysqli_query($koneksi, "SELECT JadwalID,MhswID FROM t_ppmiangketvisi WHERE  MhswID='$_SESSION[_Login]'"));
...
<tbody>";
$no = 1;								
$tampil = mysqli_query($koneksi, "SELECT * from t_ppmiangketvisi where  MhswID='$_SESSION[_Login]' and TahunID='$_SESSION[tahun_akademik]'"); 
while($r=mysqli_fetch_array($tampil)){   					         
$pertanyaan=mysqli_fetch_array(mysqli_query($koneksi, "select * from t_ppmipertanyaanvisi where PertanyaanID='$r[PertanyaanID]'"));
echo "<tr bgcolor=$warna>
<td>$no</td>				
...
for($i = 1; $i <= $JumData; $i++){
    ...
    $Jawaban  		= strfilter($_POST['Jawaban'.$i]);	
		if (!empty($PertanyaanID)){     
			$sqlcek = mysqli_query($koneksi, "SELECT * FROM t_ppmiangketvisi WHERE MhswID='$_SESSION[_Login]' and TahunID='$_SESSION[tahun_akademik]'");
			if (mysqli_num_rows($sqlcek)>0){
