...
function CetakKehadiran() {
  $mhsw = AmbilFieldx('mhsw', 'MhswID', $_SESSION['crmhswid'], '*');
  if (empty($mhsw)) echo PesanError("Mhsw Tidak Ditemukan",
...
function CetakKehadiranMhsw($mhsw) {
  ...
  $mxb = 60;
  $grs = str_pad('-', $mxc, '-').$_lf;
  $thn = AmbilOneField('tahun', "ProgramID='$mhsw[ProgramID]' and ProdiID='$mhsw[ProdiID]' and TahunID",
    $_SESSION['tahun'], 'Nama');
...
// *** Main ***
TitleApps("Laporan Kehadiran per Mahasiswa");
TampilkanPencarianMhswTahun('lap.hadir.permhsw', 'CetakKehadiran', 1);
if (!empty($crmhswid) && !empty($tahun)) $lungo();
?>
