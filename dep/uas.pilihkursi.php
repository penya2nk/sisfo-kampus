...
{	
	global $koneksi;
	$arrProdi = array();
	$jadwaluas = AmbilFieldx("jadwaluas ju left outer join jadwal j on j.JadwalID=ju.JadwalID and j.KodeID='".KodeID."'
	...
	$ruang = AmbilFieldx('ruang', "RuangID='$jadwaluas[RuangID]' and KodeID", KodeID, 'KapasitasUjian, KolomUjian');
	...
	echo "<form name='formuas' action='?ndelox=$_SESSION[ndelox]&lungo=SavData' method=POST >
...
{	
	...
	$r = mysqli_query($koneksi, $s);
	$KapasitasRuang = AmbilOneField('jadwaluas', "JadwalUASID='$juasid' and KodeID", KodeID, "Kapasitas")+0;
	$n = 0;
	while($w = mysqli_fetch_array($r))
	{	$n++;
...
{	
	global $koneksi;
	$jadwaluas = AmbilFieldx('jadwaluas', "JadwalUASID='$juasid' and KodeID", KodeID, "*");
...
{	
	...
	$jdwluas = AmbilFieldx('jadwaluas', "JadwalUASID='$id' and KodeID", KodeID, '*');
	...
	{	for($i=1; $i <=$TotalKursi; $i++)
		{	$MhswID = $_REQUEST['Urutan'.$i];
			...
			$cek = AmbilOneField('uasmhsw', "MhswID='$MhswID' and JadwalUASID='$id' and KodeID", KodeID, 'UASMhswID');
			if(!empty($cek))
			{	$s = "update uasmhsw 
						set UrutanDiRuang='$i'
	...
	$JumlahMhsw = AmbilOneField('uasmhsw', "JadwalUASID='$id' and UrutanDiRuang>0 and KodeID", KodeID, 'count(UASMhswID)'); 
