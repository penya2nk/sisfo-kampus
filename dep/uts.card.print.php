...
//end leweh add
$pdf = new PDF();
$pdf->SetTitle("Kartu UTS Mahasiswa");
...
function BuatFooter($khs, $jml, $sks, $p) {
  ...
  $MaxSKS = AmbilOneField('maxsks',
    ...
    $khs['ProdiID'], 'SKS')+0;
  // Pejabat
  $pjbt = AmbilFieldx('pejabat', "KodeID='".KodeID."' and KodeJabatan", 'KAPRODI_'.$khs['ProdiID'], "*");
  // Array Isi
  $tgl = date('d M Y');
  $arr = array();
...
function BuatIsinya($khs, $p) {
  ...
  while ($w = mysqli_fetch_array($r)) {
    $HitungMangkir = AmbilOneField('presensimhsw p 
...
function BuatHeaderDetail($p) {
  ...
  $p->Cell(15, $t, 'H.UTS', $l, 0);
  $p->Cell(15, $t, 'Tgl.UTS', $l, 0);
...
function BuatHeaderKHS($khs, $p) {
  ...
  $p->Cell($lbr, 8, "Kartu Ujian Tengah Semester", 0, 1, 'C');
  // parameter
  $prodi = $khs['_PRD'];
  $prg   = $khs['_PRG'];
  $thn   = $khs['_THN'];
