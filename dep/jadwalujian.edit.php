...
ViewHeaderApps("Info Penjadwalan Ujian", 1);

// *** Parameters ***
$jid = GainVariabelx('jid');
...
// *** Main ***
$lungo = (empty($_REQUEST['lungo']))? 'fnKonfirmasi' : $_REQUEST['lungo'];
$lungo($jid);

// *** Functions ***
function fnKonfirmasi($jid) {
  TitleApps("Jadwalkan $_SESSION[_jdwlU]");
  $w = AmbilFieldx('jadwal', 'JadwalID', $jid, '*');
  $d = AmbilFieldx('dosen', "Login = '$w[DosenID]' and KodeID", KodeID, "Nama, Gelar");
  $tgl = FormatTanggal($_SESSION['_jdwlTgl']);
  echo <<<ESD
  <table class=box width=100%>
  <form name='frmUJIAN' action='../$_SESSION[ndelox].edit.php' method=POST>
  ...
  if ($oke) {
    ...
    echo PesanError("Error",
      ...
      <input type=button name='btnKembali' value='Kembali' 
        onClick=\"location='../$_SESSION[ndelox].edit.php?jid=$jid'\"/>
      <input type=button name='btnTutup' value='Tutup'
        onClick='window.close()' />");
  }
}
