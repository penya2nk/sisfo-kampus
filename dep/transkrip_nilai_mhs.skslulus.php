...
ob_start();
$pdf = new FPDF('P');
$pdf->SetTitle("Daftar SKS Lulus Mahasiswa");
$pdf->AddPage('P');
HeaderLogo("Daftar SKS Lulus Mahasiswa", $pdf, 'P');
...
{ $mhsw = AmbilFieldx("mhsw m left outer join dosen d on m.PenasehatAkademik=d.Login and d.KodeID='".KodeID."'", 
					"m.MhswID='$MhswID' and m.KodeID", KodeID, 'm.MhswID, m.Nama, d.Nama as DSN, d.Gelar, m.ProgramID, m.ProdiID');
  $arr[] = array('NIM', ':', $mhsw['MhswID']);
  $arr[] = array('Nama', ':', $mhsw['Nama'], 'Program', ':', $mhsw['ProgramID'].' - '.AmbilOneField('program', "ProgramID='$mhsw[ProgramID]' and KodeID", KodeID, 'Nama'));
  $Dosen = (empty($mhsw['DSN']))? "(Belum diset)" : $mhsw['DSN'].', '.$mhsw['Gelar'];
  $arr[] = array('Dosen Pembimbing', ':', $Dosen, 'Program Studi', ':', $mhsw['ProdiID'].' - '.AmbilOneField('prodi', "ProdiID='$mhsw[ProdiID]' and KodeID", KodeID, 'Nama'));
