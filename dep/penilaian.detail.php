...
if ($JadwalID == 0) {
  ...
  while ($w = mysqli_fetch_array($r)) {
    PrintNilai($w['JadwalID'], $pdf);
  }
}
else PrintNilai($id, $pdf);

$pdf->Output();

function PrintNilai($jid, $p) {
...
function BuatFooter($jdwl, $p) {
  ...
  $p->Cell(130);
  $p->Cell(60, $t, "Dosen Pengampu,", 0 , 1);
  $p->Ln(10);
...
function BuatHeader($jdwl, $thn, $p) {
  ...
  $arr[] = array('Kelas / TahunID', ':', $jdwl['namaKelas'] . ' / ' . $thn['TahunID'],
