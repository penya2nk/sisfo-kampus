...
ViewHeaderApps("Pilih Kursi");

$id = sqling($_REQUEST['id']);

$lungo = (empty($_REQUEST['lungo']))? 'PilihKursi' : $_REQUEST['lungo'];
$lungo($id);
...
{	
	global $koneksi;
	$arrProdi = array();
	$jadwaluts = AmbilFieldx("jadwaluts ju left outer join jadwal j on j.JadwalID=ju.JadwalID and j.KodeID='".KodeID."'
...
<div class='table-responsive'>
	<table class=box cellspacing=2 cellpadding=4 width=600 align=center>
	...
	$ruang = AmbilFieldx('ruang', "RuangID='$jadwaluts[RuangID]' and KodeID", KodeID, 'KapasitasUjian, KolomUjian');
	...
	echo "<form name='formuts' action='?ndelox=$_SESSION[ndelox]&lungo=SavData' method=POST >
...
{	
	...
	<div class='card-header'>
	<div class='table-responsive'>
	<table id='table1' border=1 width=20%>
	...
	$r = mysqli_query($koneksi, $s);
	$KapasitasRuang = AmbilOneField('jadwaluts', "JadwalUTSID='$jutsid' and KodeID", KodeID, "Kapasitas")+0;
	$n = 0;
	while($w = mysqli_fetch_array($r))
	{	$n++;
		$a .= "<tr><td class='forbid'>$n</td><td><input type=hidden name='Kosong$n' id='Kosong$n' value='$w[MhswID]'><div class='drag t1'>$w[MhswID]<br><b>$w[Nama]</b></div></td></tr>";
	}
...
{	
	global $koneksi;
	$jadwaluts = AmbilFieldx('jadwaluts', "JadwalUTSID='$jutsid' and KodeID", KodeID, "*");
	...
	*/
	...
	<div class='card-header'>
	<div class='table-responsive'>
	<table id='table2' border=1 width=75%>";
	...
	{	echo "<td class='forbid' width=120>Kolom $j</td>"; 
	}
	echo "</tr>";
	...
	{	echo "<tr><td class='forbid'>Baris $i</td>";
		...
		{	$n++;
			if(!empty($arrSiswa[$n]))
			{	$mhsw = AmbilFieldx('mhsw', "MhswID='$arrSiswa[$n]' and KodeID", KodeID, "Nama");
				echo "<td><input type=hidden name='Urutan$n' id='Urutan$n' value='$arrSiswa[$n]'>
						  <span id='UrutanGambar$n'></span>
						  <div class='drag t1'>$arrSiswa[$n]<br><b>$mhsw[Nama]</b></div></td>";	
			}
			else echo "<td><input type=hidden name='Urutan$n' id='Urutan$n' value=''>
			              <span id='UrutanGambar$n'><img src='../img/kursi.png'></span></td>";
		}
		echo "</tr>";
	}
	echo "</table>
...
{	
	global $koneksi;
	if(empty($id)) die(PesanError('Error', "Jadwal yang dicari tidak ditemukan. <br>Harap hubungi administrator."));
	
	$jdwluts = AmbilFieldx('jadwaluts', "JadwalUTSID='$id' and KodeID", KodeID, '*');
	...
	{	for($i=1; $i <=$TotalKursi; $i++)
		{	$MhswID = $_REQUEST['Urutan'.$i];
			...
			$cek = AmbilOneField('utsmhsw', "MhswID='$MhswID' and JadwalUTSID='$id' and KodeID", KodeID, 'UTSMhswID');
			if(!empty($cek))
			{	$s = "update utsmhsw 
						set UrutanDiRuang='$i'
	...
	$JumlahMhsw = AmbilOneField('utsmhsw', "JadwalUTSID='$id' and UrutanDiRuang>0 and KodeID", KodeID, 'count(UTSMhswID)'); 
