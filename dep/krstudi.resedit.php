...
ViewHeaderApps("Edit Jadwal Tambahan", 1);
...
$lungo($md, $jid, $krsid, $krsresid, $jenis);
...
function Edit($md, $jid, $krsid, $krsresid, $jenis) {
  ...
  $w = AmbilFieldx('jadwal', "JadwalID='$jid' and KodeID", KodeID, "*");
  $w['Dosen'] = AmbilOneField('dosen', "KodeID='".KodeID."' and Login", $w['DosenID'], 'Nama');

  // Parameters
  $NamaHari = AmbilOneField('hari', 'HariID', $w['HariID'], 'Nama');
  $NamaJenisJadwal = AmbilOneField('jenisjadwal', "JenisJadwalID", $jenis, 'Nama');
  // Tampilkan
  TitleApps($jdl);
  echo <<<END
...
END;
  ...
  <form name='frmJadwalTambahan' action='../$_SESSION[ndelox].resedit.php' method=POST>
  ...
  <input type=hidden name='jenis' value='$jenis' />";
    ...
    <th class=ttl width=50>Kap.</th>
	   ...
	   $s1 = "select jr.JadwalID, jr.JadwalRefID, LEFT(jr.JamMulai, 5) as JM, LEFT(jr.JamSelesai, 5) as JS, 
		...
		{	$n1++;
			$class='ul1';
			$ada = AmbilOneField('krs', "JadwalID='$w1[JadwalID]' and KodeID", KodeID, "KRSID");
...
function Simpan($md, $jid, $krsid, $krsresid, $jenis) {
  ...
  $krs = AmbilFieldx('krs', "KRSID='$krsid' and KodeID", KodeID, "*");
