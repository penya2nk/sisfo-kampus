...
function Edit($md, $id, $jutsid) {
	global $koneksi;
  $w = AmbilFieldx('jadwal', 'JadwalID', $id, '*');
  $w['Dosen'] = AmbilOneField('dosen', "KodeID='".KodeID."' and Login", $w['DosenID'], 'Nama');
  $w['_JM'] = substr($w['JamMulai'], 0, 5);
  $w['_JS'] = substr($w['JamSelesai'], 0, 5);
  $prodi = AmbilFieldx('prodi', "ProdiID='$_SESSION[_utsProdi]' and KodeID", KodeID, "*");
  $tahun = AmbilFieldx('tahun', "TahunID='$_SESSION[_utsTahun]' and ProdiID='$_SESSION[_utsProdi]' and ProgramID='$_SESSION[_utsProg]' and KodeID", KodeID, "*");
  
  if ($md == 0) {
	$jdl = "Edit Jadwal UTS";
	$jadwaluts = AmbilFieldx('jadwaluts', 'JadwalUTSID', $jutsid, '*');
	...
	$w['UTSDosen'] = AmbilOneField('dosen', "Login='$jadwaluts[DosenID]' and KodeID", KodeID, 'Nama');
  ...
  elseif ($md == 1) {
	...
	die(PesanError("Error", "Mode tidak dikenali")); 
  }
  // Parameters
  JdwlUTSScript();
echo '
...
function GetDateOption3($value,$name){
  ...
  $opttgluts = AmbilComboTgl($w['UTSTanggal'], 'UTSTanggal');
  $optJamMulai = AmbilComboWaktu($w['UTSJamMulai'], 'UTSJamMulai');
  $optJamSelesai = AmbilComboWaktu($w['UTSJamSelesai'], 'UTSJamSelesai');
  $NamaHari = AmbilOneField('hari', 'HariID', $w['HariID'], 'Nama');
  // Tampilkan
...
<div class='table-responsive'>
  ...
  <form name='frmJadwalUTS' action='../$_SESSION[ndelox].edit.php' method=POST onSubmit="return CheckForm(this)">
...
function Simpan($md, $id, $jutsid) {
  ...
  // *** Cek semuanya dulu ***
   $cekada = AmbilFieldx('tahun', "NA='N' and TahunID='$w[TahunID]' 
            ...
            "*");
  $cek = AmbilFieldx('tahun', "NA='N' and TahunID='$w[TahunID]' 
  ...
  if(empty($cekada)){
    die(PesanError('Kesalahan Tanggal', 
  ...
  // Ambil data MK
  $mk = AmbilFieldx('mk', "MKID", $w['MKID'], "Nama,MKKode,KurikulumID,SKS,Sesi");
...
{ $uts = AmbilFieldx('tahun', "TahunID='$w[TahunID]' and ProgramID='$w[ProgramID]' and ProdiID='$w[ProdiID]' and KodeID", KodeID, "LEFT(TglUTSMulai, 10) as _TglUTSMulai, LEFT(TglUTSSelesai, 10) as _TglUTSSelesai");
...
function DaftarkanMhswKeRuangUTS($w, $JadwalUTSID)
{
global $koneksi;
	//echo "JADWALUTSID=$JadwalUTSID ,JADWALID=$w[JadwalID]<br>";
	if($JadwalUTSID+0 != 0)
    {   
		...
		{   //Cek apakan mahasiswa sudah terdaftar
			$ada = AmbilOneField('utsmhsw', "MhswID='$w2[MhswID]' and JadwalUTSID='$JadwalUTSID' and KodeID", KodeID, "UTSMhswID");
			if(empty($ada))
			{	$s1 = "insert into utsmhsw
				(KodeID, MhswID, JadwalUTSID, TahunID, RuangID, TanggalBuat, LoginBuat)
				values
				('".KodeID."', '$w2[MhswID]', '$JadwalUTSID', '$w[TahunID]', '$w[UTSRuangID]', now(), '$_SESSION[_Login]')";
				$r1 = mysqli_query($koneksi, $s1);
			}
		}
	}
	else
	die(PesanError("Error", "Tidak ditemukan Jadwal UTS yang dimaksud. Harap menghubungi yang berwenang."));
}
...
function GetDateOption2($dt, $nm='dt',$loc='') {
  ...
  $_yr = AmbilComboNomor(1930, Date('Y')+2, $arr[0]);
...
function JdwlUTSScript() {
  ...
  function CariRuang(ProdiID, frm) {
    if (eval(frm + ".UTSRuangID.value != ''")) {
      ...
      showRuang(ProdiID, frm, eval(frm +".UTSRuangID.value"), 'cariruang');
      toggleBox('cariruang', 1);
    }
  }
  function CariDosen(ProdiID, frm) {
    if (eval(frm + ".UTSDosen.value != ''")) {
      eval(frm + ".UTSDosen.focus()");
      showDosen(ProdiID, frm, eval(frm +".UTSDosen.value"), 'caridosen');
