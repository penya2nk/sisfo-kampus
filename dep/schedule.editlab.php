...
function GetDateOption3($value,$name){
  ...
  {	if($md == 0) $jdl = "Edit Jadwal Ekstra";
	...
	else die(PesanError('Fatal Error', "Mode Edit tidak diketahui"));
	
	$w = AmbilFieldx('jadwal j left outer join prodi prd on j.ProdiID=prd.ProdiID
  ...
  {	  if ($md == 0) {
		...
		$w = AmbilFieldx('jadwal jr left outer join jadwal j on jr.JadwalRefID=j.JadwalID
  ...
  $opthari = AmbilCombo2('hari', "Nama", 'HariID', $w['HariID'], '', 'HariID');
  ...
  $optjenisjadwal = AmbilCombo2('jenisjadwal', "concat(JenisJadwalID, ' - ', Nama)", 'JenisJadwalID', $w['JenisJadwalID'], "NA='N' and Tambahan='Y'", 'JenisJadwalID', 0, 0);
  // Parameters
  JdwlScript();
  // Tampilkan
...
echo '
  ...
  <form name='frmJadwal' action='../$_SESSION[ndelox].editlab.php' method=POST onSubmit="return CheckForm(this)">
...
function KembaliScript($md, $id, $resid, $w) {
  ...
  <script>
  function Kembali() {
    window.location="../$_SESSION[ndelox].editlab.php?lungo=Edit&md=$md&id=$id&resid=$resid&Kembali=1&KuliahTanggal=$w[KuliahTanggal]"+
