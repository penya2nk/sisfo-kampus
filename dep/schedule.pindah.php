...
echo <<<SCR
  <script src="../$_SESSION[ndelox].pindah.script.js"></script>
SCR;

ViewHeaderApps("Pindahkan Peserta Kuliah", 1);
// *** Parameters ***
...
// *** Main ***
TitleApps("Pindahkan Peserta Kuliah");
$lungo = (empty($_REQUEST['lungo']))? 'Pindahkan' : $_REQUEST['lungo'];
$lungo($JadwalID, $jdwl);
...
function Pindahkan($JadwalID, $jdwl) {
  ...
  <form name='frmPindah' action='../$_SESSION[ndelox].pindah.php' method=POST>
...
function Simpan($JadwalID, $jdwl) {
  ...
  if (empty($_JadwalID))
    die(PesanError('Error',
      ...
      Opsi: <input type=button name='Kembali' value='Kembali'
        onClick=\"location='../$_SESSION[ndelox].pindah.php?JadwalID=$JadwalID'\" />
  ...
  // Params
  $_j = AmbilFieldx('jadwal', 'JadwalID', $_JadwalID, '*');
  // Jika ada mhsw-nya
  $psn = array();
  for ($i = 1; $i <= $KRS; $i++) {
    $KRSID = $_REQUEST['KRSID_'.$i]+0;
    if ($KRSID > 0) {
      $_krs = AmbilFieldx('krs', 'KRSID', $KRSID, "MhswID, JadwalID, MKID, TahunID");
      // Cek dulu
      $ada = AmbilOneField('krs', "KodeID='".KodeID."' and TahunID='$_krs[TahunID]' and JadwalID=$_j[JadwalID] and MhswID",
        $_krs['MhswID'], 'KRSID')+0;
      if ($ada > 0) {
  ...
  echo "<script>opener.location='../index.php?ndelox=$_SESSION[ndelox]&lungo=';</script>";
...
function HitungPeserta($id) {
  $jml = AmbilOneField('krs', "NA='N' and JadwalID", $id, 'count(KRSID)')+0;
