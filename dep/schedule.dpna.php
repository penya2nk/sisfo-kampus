...
$pdf->Output();
ob_end_flush(); // if error enable this
...
global $koneksi;
  ...
  $s = "select k.MhswID, LEFT(m.Nama, 24) as NamaMhsw
    ...
    where k.JadwalID = '$jdwl[JadwalID]'
      and h.Bayar+h.Potongan >= h.Biaya+h.Tarik
    order by k.MhswID";
  $r = mysqli_query($koneksi, $s); $n = 0; $ttk = '........';
  while ($w = mysqli_fetch_array($r)) {
    $n++;
...
function BuatHeaderDPNA($jdwl, $thn, $p) {
  $t = 4;
  $p->AddPage('L');
  HeaderLogo('Daftar Lengkap Mahasiswa dan Nilai', $p, 'P');
  // Kop 2
  $p->SetXY(170, 10);
