...
<?php	
	$lungo = (empty($_REQUEST['lungo']))? 'ListKelas' : $_REQUEST['lungo'];
	$lungo($ProdiUSMID, $_usm_ruang, $gel);
	// *** Functions ***
	function ListKelas($ProdiUSMID, $ruangid, $gel)
	{	
		$prodiusm = AmbilFieldx("prodiusm pu left outer join pmbusm pu2 on pu.PMBUSMID=pu2.PMBUSMID and pu2.KodeID='".KodeID."'", 'pu.ProdiUSMID', $ProdiUSMID, 'pu.*, pu2.Nama, pu2.CaraPenempatan');
		...
		echo "<table class=bsc cellspacing=1 align=center>
				<form name='USM' action='?ndelox=$_SESSION[ndelox]&lungo=SavData' method=POST>
		...
		foreach ($arrRuang as $a) {
			...
			$NamaRuang = AmbilOneField('ruang', "RuangID='$a' and KodeID", KodeID, 'Nama');
			echo "<td $sel><a href='?ndelox=$_SESSION[ndelox]&lungo=&_usm_ruang=$a&PMBID=$PMBID&ProdiUSMID=$ProdiUSMID&gel=$gel'>$NamaRuang</a></td>";
			$ruangcount++;
		}
		echo "</table>";	
		
		// Gambar tabelnya.
		$ruang = AmbilFieldx('ruang', "RuangID='$_usm_ruang' and KodeID", KodeID, 'KapasitasUjian, KolomUjian');
		...
		{	echo "<tr>";
			...
			{	$n++;$class='';$entry=''; $tambahan='';
				if(empty($arrSiswa[$n]))
				{	$entry = "<img src='../img/kursi.png'>
								<br><sup>Kolom $j, Baris $i</sup>";
				}
				else
				{	
					$cama = AmbilFieldx('pmb', "PMBID='$arrSiswa[$n]' and KodeID", KodeID, '*');
	...
	{	for($i = 1; $i <= $Jumlah; $i++)
		{	$_index = $_REQUEST["Index$i"];
			...
			{	
				...
				$r = mysqli_query($koneksi, $s);
				//$AplikanID = AmbilOneField('ruangusm ru left outer join pmb p on ru.PMBID=p.PMBID', "ru.RuangUSMID='$_index' and ru.KodeID", KodeID, "p.AplikanID");
				$PMBID = AmbilOneField('ruangusm', "RuangUSMID='$_index' and KodeID", KodeID, "PMBID");
				$AplikanID = AmbilOneField('pmb', "PMBID='$PMBID' and KodeID", KodeID, "AplikanID");
				SetStatusAplikan('USM', $AplikanID);
			}
			
		}
	}
	echo "<script>location='../$_SESSION[ndelox].frame.php?lungo=&ProdiUSMID=$ProdiUSMID&gel=$gel'</script>";
}	
?>
