...
ViewHeaderApps("Proses Biaya Mahasiswa");

$_byaJumlah = GainVariabelx('_byaJumlah');
$_byaProses = GainVariabelx('_byaProses');
...
function JalankanProses($_byaJumlah, $_byaProses) {
  ...
  if ($BIPOTID > 0 && $Total > 0) {
    ...
    $prodi = AmbilOneField('prodi', "KodeID='".KodeID."' and ProdiID", $ProdiID, 'Nama');
    $smt = AmbilOneField('semester', 'Semester', $_SESSION['_byaSemester'], 'Nama');
    $smt = substr($smt, 0, 6);
