...
function TampilkanPenarikan($id, $p) {
  ...
  // Datanya
  $p->SetFont('Helvetica', '', 10);
  $w = AmbilFieldx('bayarmhsw', "BayarMhswID='$id' and KodeID", KodeID, 
    "*, date_format(Tanggal, '%d %M %Y') as _Tanggal, format(Jumlah, 2) as _Jumlah");
  $Nama = AmbilOneField('mhsw', "MhswID='$w[MhswID]' and KodeID", KodeID, 'Nama');
  ...
  $arr[] = '';
  // Footer
  $Penerima = AmbilOneField('karyawan', "Login='$w[LoginBuat]' and KodeID", KodeID, 'Nama');
...
function TampilkanHeader($id, $p) {
  ...
  $byr = AmbilFieldx('bayarmhsw', "BayarMhswID='$id' and KodeID", KodeID, "*");
  $mhsw = AmbilFieldx('mhsw', "MhswID = '$byr[MhswID]' and KodeID", KodeID, '*');
