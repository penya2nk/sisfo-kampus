...
// *** Parameters ***
$Kelipatan = GainVariabelx('Kelipatan');
$gel = $_REQUEST['gel'];
$tahun = AmbilOneField('pmbperiod', "PMBPeriodID='$gel' and KodeID", KodeID, 'Tahun');
$prevtahun = $tahun-1;
$nexttahun = $tahun+1;
$urutan = AmbilOneField('pmbperiod', "PMBPeriodID='$gel' and KodeID", KodeID, 'Urutan');

$lungo = (empty($_REQUEST['lungo']))? 'ParameterSetup' : $_REQUEST['lungo'];
$lungo($prevtahun, $tahun, $nexttahun, $urutan, $gel);
...
{	if(empty($tahun)) echo "<b>Tahun $gel tidak terdaftar. Harap menghubungi Kepala PMB untuk membenarkannya</b>";
	else
	{
		...
		echo "<p><table class=bsc cellspacing=1 width=400 border=1 align=center>
				<form action='../$_SESSION[ndelox].faktapmb.php' method=POST onSubmit=\"return CheckForm(this)\">
				<input type=hidden name='lungo' value='CetakLaporan' />
...
function CetakLaporan($prevtahun, $tahun, $nexttahun, $urutan, $gel)
{
	...
	while($w = mysqli_fetch_array($r)) $arrStatusAplikan[] = $w['StatusAplikanID'];
...
function BuatTotal($arrStatusAplikan, $arrPrevTotal, $arrCurTotal, $lebar, $tahun, $p) {
  ...
  foreach ($arrStatusAplikan as $stat) {
	...
	//$totaltarget = AmbilOneField('pmbperiod', "Tahun", $tahun, 'sum(Target'.$stat.')');
...
function BuatEntry($Urutan, $arrStatusAplikan, &$arrPrevTotal, &$arrCurTotal, $t, $lebar, $prevtahun, $tahun, $p) {
  ...
  foreach ($arrStatusAplikan as $stat) {
	...
	//$target = AmbilOneField('pmbperiod', "Tahun='$tahun' and Urutan", $Urutan, 'Target'.$stat);
...
{	
	foreach ($arrStatusAplikan as $stat) {
		$jml = AmbilOneField("statusaplikanmhsw sam left outer join aplikan a on sam.AplikanID=a.AplikanID
...
{	
	...
	$s = "select InfoID, Nama from sumberinfo order by InfoID";
	$r = mysqli_query($koneksi, $s);
	$count = 0;
	$totfg = 0;
	$totnfg = 0;
	...
	{	$count++;
		...
		$fg = AmbilOneField("aplikan a left outer join pmbperiod p on a.PMBPeriodID=p.PMBPeriodID and a.KodeID=p.KodeID and p.Tahun='$tahun'", 
							"INSTR(concat(',',a.SumberInformasi,','), concat(',',$w[InfoID],','))>0 and (a.TahunLulus='$prevtahun' or a.TahunLulus='$tahun') and a.KodeID", KodeID, 
							"count(a.AplikanID)");
		$totfg +=$fg;
		$p->Cell($lebar, $t, $fg, 1, 0, 'C');
		$nfg = AmbilOneField("aplikan a left outer join pmbperiod p on a.PMBPeriodID=p.PMBPeriodID and a.KodeID=p.KodeID and p.Tahun='$tahun'", 
