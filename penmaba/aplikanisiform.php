...
if ($_SESSION['_LevelID'] == 1) {
  $AplikanID = GainVariabelx('_pmbLoginAplikanID');
}
...
$oke = BolehAksesData($_SESSION['_Login']);
  ...
  if ($oke) {
    ...
    else $lungo($gel, $AplikanID);
  }
  
// *** Functions ***

function Main($gel, $aplikanid)
{	$ButuhGanti = AmbilOneField('aplikan', 'AplikanID', $aplikanid, 'PasswordBaru');
...
{	echo Info("Password Baru", 
		...
		<br>
		<a href='#' onClick=\"location='?ndelox=pmb/aplikanpassword'\">Setup Password Baru</a>");
}	
...
function BolehAksesData($aplikanid) {
  ...
  else if(AmbilOneField('pmb', "AplikanID='$aplikanid' and KodeID", KodeID, 'LulusUjian') == 'Y') {
...
ESD;
  ...
  <form name='data' action='?ndelox=$_SESSION[ndelox]' method=POST onSubmit="return CheckForm(this)" />
