...
function Edit($md, $prd, $id) {
  ...
  if ($md == 0) {
    $jdl = "Edit USM";
    $w = AmbilFieldx('prodiusm', 'ProdiUSMID', $id, '*');
  }
  elseif ($md == 1) {
    $jdl = "Tambah USM";
    $w = array();
    $w['Urutan'] = AmbilOneField('prodiusm', "KodeID='".KodeID."' and ProdiID", $prd, "max(Urutan)")+1;
  ...
  $TanggalUjian = AmbilComboTgl($w['TanggalUjian'], 'TanggalUjian');
  $JamMulai = AmbilComboWaktu($w['JamMulai'], 'JamMulai');
  $JamSelesai = AmbilComboWaktu($w['JamSelesai'], 'JamSelesai');
  $optusm = AmbilCombo2('pmbusm', "concat(PMBUSMID, ' - ', Nama)", 'PMBUSMID', @$w['PMBUSMID'], "KodeID='".KodeID."'", 'PMBUSMID');
  ...
  // ambil gelombang euy
	...
	while ($w3 = mysqli_fetch_array($q3)){
  ...
  <form nama=formusm action='../$_SESSION[ndelox].prodiusm.edit.php' method=POST onSubmit='return CheckForm($counting,$w2[_fromday],$w2[_frommonth],$w2[_fromyear],$w2[_today],$w2[_tomonth],$w2[_toyear],$w2[PMBPeriodID])'>
...
function Simpan($md, $prd, $id) {
  ...
  $w['PMBPeriodID'] = AmbilOneField('pmbperiod', "KodeID='".KodeID."' and NA", 'N', "PMBPeriodID");
  $Ruang = $_REQUEST['Ruang'];
  
  $cekada = AmbilFieldx('pmbperiod', "KodeID='".KodeID."' and 
            left(UjianMulai,10)<= '$w[TanggalUjian]' and 
            left(UjianSelesai,10)>= '$w[TanggalUjian]' and NA", "N", "*");
  
  $cek = AmbilFieldx('pmbperiod', "KodeID='".KodeID."' and NA","N",
...
<div class='table-responsive'>
		...
		$a .= "
			<tr><td colspan=2 align=center><input type=button name='Gabung' value='Satukan dengan jadwal ini' 
												onClick=\"location='?ndelox=$_SESSION[ndelox]&rnd=qo4qo3o480badaoiabsdfb224&lungo=GabungkanProdi&md=$md&prd=$prd&id=$id&idtujuan=$w1[ProdiUSMID]'\"></td></tr>";
...
$idtujuan = $_REQUEST['idtujuan'];
	if($md == 0)
	{	$ProdiID = AmbilOneField('prodiusm', "ProdiUSMID='$id' and KodeID", KodeID, 'ProdiID');
	...
	$arrProdi = explode('|', AmbilOneField('prodiusm', "ProdiUSMID='$idtujuan' and KodeID", KodeID, 'ProdiID'));
	$arrProdi[] = $prd;
...
<SCRIPT>
  function ttutup() {
    opener.location='../index.php?ndelox=$_SESSION[ndelox]&lungo=prodiusm';
    self.close();
    return false;
  }
</SCRIPT>
