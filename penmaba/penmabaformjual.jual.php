...
// *** Main ***
$lungo = (empty($_REQUEST['lungo']))? 'Jualan' : $_REQUEST['lungo'];
$lungo($gel, $id, $tn, $tnpmb);

function Jualan($gel, $id, $tn) {
	global $koneksi;
  TitleApps("Jual Formulir - $gel");
  $frm = AmbilFieldx('pmbformulir', 'PMBFormulirID', $id, '*');
  $hrg = number_format($frm['Harga']);
  
  if(!empty($tn) and $tn!='' and $tn!=0)
	{	
		$grey = "D3D3D3";
  ...
  {	$arrPrasyarat = explode('|', $frm['PrasyaratExtra']);
	...
	{	$n++;
		...
		$pmbformsyarat = AmbilFieldx('pmbformsyarat', "PMBFormSyaratID='$arr[0]' and KodeID", KodeID, "*");
		if($arr[1] == 'Y')
  ...
  <form action='../$_SESSION[ndelox].jual.php' method=POST onSubmit=\"return CheckForm(this)\">
  ...
  <tr><td class=inp nowrap>Bukti Setoran:</td>
      <td class=ul1><input type=text name='BuktiSetoran'
        size=20 maxlength=50 required /> <br />(Untuk Pendaftaran Online Bukti Setoran Ditulis ONLINE)</td>
      </tr>
  ...
  <script>
	...
	{	
		...
		if(temp!='')
		{
			...
			lnk = '../$_SESSION[ndelox].cari.php?gel='+gel+'&id='+id+'&n='+temp;
			//alert(lnk);
			win2 = window.open(lnk, '', 'width=600, left=430, top=100,  height=600, scrollbars, status, resizable');
...
function Simpan($gel, $id, $tn) {
		...
		{   if($pmbformsyarat['AdaScript'] == 'Y')
			{
				...
				{	$_Script = str_replace('=INPUT=', "'$arr[2]'", $pmbformsyarat['Script']);
					$cari = AmbilOneField('aplikan', "AplikanID='$AplikanID' and $_Script and KodeID", KodeID, "AplikanID");
  ...
  {	// Buat nomer baru
	  ...
	  $nomer = $gel . $nomer;
	  $akhir = AmbilOneField('pmbformjual',
	  ...
	  echo "<script>opener.location='../index.php?ndelox=$_SESSION[ndelox]';</script>";
	  ...
	  <tr><td class=ul1 colspan=2 align=center>
		  <input type=button name='Tutup' value='Tutup' onClick=\"javascript:ttutup('$_SESSION[ndelox]')\" />
		  <input type=button name='CetakKwitansi' value='Cetak Kwitansi'
			onClick=\"location='../$_SESSION[ndelox].kwitansi.php?id=$baru' \" />
		  </td>
		  </tr>
	  </table>";
   }
