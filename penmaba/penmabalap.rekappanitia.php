...
// *** Parameters ***
$Fee = GainVariabelx('Fee');
$gel = $_REQUEST['gel'];
$tahun = AmbilOneField('pmbperiod', "PMBPeriodID='$gel' and KodeID", KodeID, 'Tahun');
$prevtahun = ($tahun-1);

$lungo = (empty($_REQUEST['lungo']))? 'CetakLaporan' : $_REQUEST['lungo'];
$lungo($prevtahun, $tahun, $gel);
...
function TampilkanIsinya($prevtahun, $tahun, $arrGelombang, $arrPejabatID, $p) {
  ...
  for ($i = 0; $i < sizeof($arrPejabatID); $i++) {
    ...
    $namapejabat = AmbilOneField('pejabat', 'PejabatID', $arrPejabatID[$i], 'NamaInisial');
...
function BuatHeaderTabel($arrPejabatID, $arrGelombang, $lebar, $prevtahun, $tahun, $p ) {
  ...
  $p->Cell($lebar, $t, '', 'LR', 0);
  foreach ($arrGelombang as $stat) {
	$tglujian = AmbilOneField('pmbperiod', "Tahun='$tahun' and Urutan='$stat' and KodeID", KodeID, "date_format(UjianMulai, '%d-%b-%Y')"); 
...
{	
	foreach ($arrGelombang as $stat) {
		$jml = AmbilOneField("wawancara w left outer join pmbperiod pp on w.PMBPeriodID=pp.PMBPeriodID and w.KodeID=pp.KodeID",
	...
	foreach ($arrGelombang as $stat) {
		$jml = AmbilOneField("pmbperiod pp", "INSTR(concat('~',pp.TechnicalSupportID,'~'),concat('~',$PejabatID,'~'))>0 and pp.Tahun='$tahun' and pp.Urutan='$stat' and pp.KodeID", KodeID, 'count(pp.PMBPeriodID)')+0;
