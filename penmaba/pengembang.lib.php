...
function AmbilCombo2($_table, $_field, $_order='', $_default='', $_where='', $_value='', $not=0) {
  global $strCantQuery;
...
function AmbilComboTgl($dt, $nm='dt') {
  ...
  $_yr = AmbilComboNomor(1945, Date('Y')+1, $arr[0]);
...
function AmbilFieldx($_tbl, $_key, $_value, $_results) {
  global $strCantQuery;
...
function TampilkanSubMenu($ndelox, $arr=array(), $pref='', $tok='') {
  $sb = '';
...
function TampilkanSubMenu2($ndelox, $arr=array(), $pref='', $tok='') {
  $sb = '';
  for ($i=0; $i<sizeof($arr); $i++) {
    $r = explode('->', $arr[$i]);
    //$cl = ($r[1] == $tok)? "class=menuaktif" : "class=menuitem";
...
function GetNextPMBID($prodi='') {
  ...
  $pmbmx = AmbilOneField('pmbweb', "PMBID like '$pmbaktif%' and NA", 'N', "right(max(PMBID),3)");
...
function GetNextPSSBID($tahun) {
  global $_PMBDigit;
  $mx = AmbilOneField('pssb', "PSSBID like '$tahun%' and NA", 'N', "max(PSSBID)");
...
function GetNextNIM($ProdiID, $kdp='') {
  // Ambil Setup NIM
  $stp = AmbilOneField('prodi', 'ProdiID', $ProdiID, 'FormatNIM');
  $tmp = $stp;
...
function GetNextPMBFormulirID($pmbfid=1) {
  ...
  $pmbmx = AmbilOneField('pmbformjual', "PMBFormJualID like '".$pmbaktif.$panjang."' and NA",
    'N', "max(PMBFormJualID)");
...
function GetNextBPM() {
  ...
  $bpmmx = AmbilOneField('bayarmhsw', "BayarMhswID like '".$thn.$panjang."' and NA", 
    'N', "max(BayarMhswID)");
  $bpmcnt = str_replace($thn, '', $bpmmx)+1;
  $bpmcnt = $thn.str_pad($bpmcnt, $_BPMDigit, '0', STR_PAD_LEFT);
  return $bpmcnt;
}
function DaftarProdi($ndelox) {
  ...
  while ($w = mysqli_fetch_array($r)) {
    ...
    $a .= "<tr><td align=center>$ki</td>
      <td $c>$w[ProdiID]</td>
      <td $c><a href='?$ndelox&prodiid=$w[ProdiID]'>$w[Nama]</a></td>
      <td $c>$w[NA]</td>
...
function TampilkanPeriodePMB($ndelox='') {
  global $arrID;
...
function TampilkanPeriodePMBLaporan($ndelox) {
...
function TampilkanPilihanProdi($ndelox='', $lungo='', $pref='', $token='') {
  global $arrID;
  ...
  $opt = AmbilCombo2("prodi", "concat(ProdiID, ' - ', Nama)", "ProdiID", $_SESSION['prodi'], "KodeID='$arrID[Kode]' and ProdiID in ($_prodi)", 'ProdiID');
...
function TampilkanPilihanProdiProgram($ndelox='', $lungo='', $pref='', $token='') {
  global $arrID;
...
function TampilkanPilihanFakultas($ndelox='', $lungo='', $pref='', $token='') {
  global $arrID;
  $optfak = AmbilCombo2('fakultas', "concat(FakultasID, ' - ', Nama)", "FakultasID", $_SESSION['fakid'], "KodeID='$arrID[Kode]'", 'FakultasID');
  echo "<p><table class=box cellspacing=1 cellpadding=4>
  <form action='?' method=POST>
  <input type=hidden name='ndelox' value='$ndelox'>
  <input type=hidden name='lungo' value='$lungo'>
...
function EditUserProfile($tbl, $lgn, $ndelox, $lungo, $gosval) {
...
function SaveUserProfile() {
  ...
  $r = mysqli_query($s);
  echo Info("Berhasil", "Profile berhasil diupdate");
}
function TampilkanPilihanInstitusi($ndelox) {
  $opt = AmbilCombo2('identitas', "concat(Kode, ' - ', Nama)", 'Kode', $_SESSION['KodeID'], '', 'Kode');
  echo "<p><table class=box cellspacing=1 cellpadding=4>
  <form action='?' method=POST>
  <input type=hidden name='ndelox' value='$ndelox'>
...
function TampilkanPilihanKurikulum() {
  ...
  $optprodi = AmbilCombo2("prodi", "concat(ProdiID, ' - ', Nama)", "ProdiID", $_SESSION['prodi'], "KodeID='$arrID[Kode]' and ProdiID in ($_prodi)", 'ProdiID');
...
function AmbilCekBox($table, $key, $Fields, $Label, $Nilai='', $Separator=',', $whr = '') {
...
function TampilkanPencarianMhsw($ndelox='mhswakd', $lungo='CariMhsw', $btn=2) {
  global $arrID;
...
function TampilkanPencarianMhswTahun($ndelox='mhswakd', $lungo='CariMhsw', $btn=2) {
  global $arrID;
  $strbtn = "<input type=submit name='crmhsw' value='NPM'>";
  $strbtn .= ($btn >=2 )? "<input type=submit name='crmhsw' value='Nama'>" : '';
  echo "<p><table class=box cellspacing=1 cellpadding=4>
...
function TampilkanPencarianCAMA($ndelox='') {
  global $arrID;
  echo "<p><table class=box cellspacing=1 cellpadding=4>
  <form action='?' method=POST>
  <input type=hidden name='ndelox' value='$ndelox'>
