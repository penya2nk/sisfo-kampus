...
ViewHeaderApps("GELOMBANG PENERIMAAN MAHASISWA BARU");
global $themas;
echo $themas;
...
function Edit($md, $id, $bck) {
  if ($md == 0) {
    $jdl = "Edit Gelombang";
    $w = AmbilFieldx('pmbperiod', "KodeID='".KodeID."' and PMBPeriodID", $id, "*");
  ...
  // Parameters
  $arrTahun = array();
  for($i = date('Y')+3; $i >= 2000; $i--) { $arrTahun[] = $i; }
  ...
  <form action='../$_SESSION[ndelox].gelombang.edit.php' method=POST onSubmit=\"return CheckForm(this)\">
  ...
  $NA = ($_REQUEST['NA'] == 'Y')? 'Y' : 'N';
    ...
    if ($md == 0) {
	  ...
	  elseif ($md == 1) {
		...
		$ada = AmbilFieldx('pmbperiod', "KodeID='".KodeID."' and PMBPeriodID", $id, '*');
		if (empty($ada)) {
		  if ($NA == 'N') DisablePeriod();
		  
		  $ada2 = AmbilOneField('pmbperiod', "Tahun='$Tahun' and Urutan", $Urutan, 'PMBPeriodID');
		  if(empty($ada2))
		  {
		  ...
		  {	  die(PesanError('Error', "Gelombang untuk Tahun <b>$Tahun</b> dan Urutan <b>$Urutan</b> telah digunakan.<br />
					Kode yang digunakan adalah: $ada. </br>
			  <hr size=1 color=silver />
			  Opsi: <input type=button name='Tutup' value='Tutup' onClick=\"window.close()\" />"));
		  }
		}
		else die(PesanError('Error', "Kode Gelombang <b>$id</b> telah digunakan.<br />
		  Coba gunakan kode gelombang yang lain.
		  <hr size=1 color=silver />
		  Opsi: <input type=button name='Tutup' value='Tutup' onClick=\"window.close()\" />"));
	  }
	  else die(PesanError('Error', "Terjadi kesalahan mode edit.<br />Mode edit <b>$md</b> tidak dikenali."));
