...
{	$t = 5; $lebar = 15;
	...
	{	$count++;
		...
		$fg = AmbilOneField("pmb b left outer join pmbperiod p on b.PMBPeriodID=p.PMBPeriodID and b.KodeID=p.KodeID and LEFT(p.PMBPeriodID, 4)='$tahun'", 
							"INSTR(concat(',',b.SumberInfo,','), concat(',',$w[InfoID],','))>0 and (b.TahunLulus='$prevtahun' or b.TahunLulus='$tahun') and b.KodeID", KodeID, 
							"count(b.PMBID)");
		$totfg +=$fg;
		$p->Cell($lebar, $t, $fg, 1, 0, 'C');
		$nfg = AmbilOneField("pmb b left outer join pmbperiod p on b.PMBPeriodID=p.PMBPeriodID and b.KodeID=p.KodeID and LEFT(p.PMBPeriodID, 4)='$tahun'", 
