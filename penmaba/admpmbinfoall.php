...
<div class="card-header">
	  ...
	  <?php 
		if (isset($_GET['tahun'])){     
			echo "<b style='color:green;font-size:18px'>Data PMB Tahun $_GET[tahun] </b>| <a  href='print_report/datapmball.php?tahun=$_GET[tahun]&prodi=$_GET[prodi]' target='_BLANK'>Exp PDF |</a>"; 
			echo "<a  href='print_reportxls/datapmballxls.php?tahun=$_GET[tahun]&prodi=$_GET[prodi]' target='_BLANK'> Exp XLS |</a>"; 
		}  
	  ?></b>
	</h3>
	
	<div class="form-group row">
					<label class="col-md-9 col-form-label text-md-right"><b style='color:purple'>TAHUN</b></label>
					<div class="col-md-2">	  
					<form style='margin-right:5px; margin-top:0px' class='pull-right' action='' method='GET'>
					<input type="hidden" name='ndelox' value='penmaba/admpmbinfoall'> 
						<select name='tahun'  class='form-control form-control-sm' onChange='this.form.submit()'>
							<?php 
								echo "<option value=''>- Pilih Tahun PMB -</option>";
								$tahun = mysqli_query($koneksi, "SELECT * FROM t_tahunnormal order by Tahun Desc limit 8"); //and NA='N'
...
<div class='table-responsive'>
	  ...
	  <?php
		if (isset($_GET['tahun'])){
			$tampil = mysqli_query($koneksi, "SELECT * FROM pmb where left(PMBPeriodID,4)='".strfilter($_GET['tahun'])."' order by PMBID desc");                    							
			$no++;
		    ...
		    if ($r['RegUlang']=='Y'){
			  ...
			  echo "<tr $c>
			  <td>$no</td>					  					 
			  <td><a href='?ndelox=penmaba/admpmbreg&act=viewdetail&PMBID=$r[PMBID]&program=$r[ProgramID]&prodi=$r[ProdiID]'>$r[PMBID]</a></td>
			  <td>$Nama</td>
...
<a href='index.php?ndelox=pmb/admpmbinfoall&tahun=$_GET[tahun]&formulir=$_GET[formulir]&program=$_GET[program]'><button type='button' class='btn btn-default pull-right'>Cancel</button></a>
...
if ($cek>0){
   echo "<script>document.location='index.php?ndelox=pmb/admpmbinfoall&program=$_GET[program]&prodi=$_GET[prodi]&tahun=$_GET[PMBPeriodID]';</script>";
}else{
   ...
   $sqlcek2=mysqli_query($koneksi, "SELECT MhswID from mhsw where MhswID='".strfilter($_GET['$NewID'])."'");
		...
		if ($cek2>0){
			...
			mysqli_query($koneksi, "INSERT into mhsw(MhswID,
													 Login,
													 Password,
													 PMBID,
													 PMBFormJualID,
...
elseif($_GET[act]=='orasido'){  
   ...
   echo "<script>document.location='index.php?ndelox=pmb/admpmbinfoall&prodi=$_GET[prodi]&tahun=$_GET[PMBPeriodID]';</script>";
}
