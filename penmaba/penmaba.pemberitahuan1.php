...
$period = AmbilFieldx('pmbperiod', 'PMBPeriodID', $_REQUEST['pmbaktif'], '*');
if (!empty($pmbid)) {
  // buat file
...
function GetDiskon($pmb, $bipotid, $Diskon) {
  ...
  $r = mysqli_query($koneksi, $s); $total = 0;
...
function GetBipot2($pmb, $bipotid, &$total) {
  ...
  $a = ''; $n = 0; $total = 0;
  while ($w0 = mysqli_fetch_array($r0)) {
    if ($w0['Jumlah'] == 0) {}
...
function InsertBIPOT($n, $w, &$tot, $bipotid, $pmb) {
  ...
  if ($w['DefJumlah'] > 1) {
    // Jika BPP SKS
    if ($w['BIPOTNamaID'] == 5) {
      $detbipot = AmbilFieldx('bipot', "BIPOTID", $bipotid, "*");
      $_prd = $detbipot['ProdiID'];
      $w['DefJumlah'] = AmbilOneField('prodi', 'ProdiID', $_prd, "DefSKS");
    }
