...
{	
	foreach ($arrRevStatusAplikan as $stat) {
		$prevjml = AmbilOneField('aplikan a left outer join pmbperiod p on a.PMBPeriodID=p.PMBPeriodID and a.KodeID=p.KodeID',
...
{	
	foreach ($arrRevStatusAplikan as $stat) {
		$jml = AmbilOneField('aplikan a left outer join pmbperiod p on a.PMBPeriodID=p.PMBPeriodID and a.KodeID=p.KodeID',
