...
{	
	...
	{	if($w['PMBFormulirID']=='' or !isset($w['PMBFormulirID']) or $w['PMBFormulirID']==NULL)
		...
		{	
			...
			$NamaSekolah = AmbilOneField('asalsekolah', 'SekolahID', $w['AsalSekolah'], 'Nama');
			if(empty($NamaSekolah) or $NamaSekolah=='') { $NamaSekolah = AmbilOneField('perguruantinggi', 'PerguruanTinggiID', $w['AsalSekolah'], 'Nama');	}
		    
			$JumlahPilihan = (empty($w['PMBFormulirID']))? 3 : AmbilOneField('pmbformulir', "PMBFormulirID='$w[PMBFormulirID]' and KodeID", KodeID, 'JumlahPilihan');
