...
function CariDosen() {
  ...
  //echo "<a href=?ndelox=$_SESSION[ndelox].cetak>Cetak</a>";
  DaftarDosen($_SESSION['ndelox'], "gos=DsnEdt&md=0&dsnid==Login=", "NIDN,Nama,Gelar,Homebase,Telephone");
}
...
function FormLast(){
  $optstts = AmbilCombo2('statusdosen', "concat(StatusDosenID, ' - ', Nama)", "StatusDosenID", $_SESSION['last'], '', "StatusDosenID");
...
function DosenTambah($ndelox='', $gos='DsnAddSav', $sub='') {
  if (empty($ndelox)) $ndelox = $_SESSION['ndelox'];
  $md = $_REQUEST['md']+0;
  if ($md == 0) {
    $w = AmbilFieldx('dosen', 'Login', $_REQUEST['dsnid'], '*');
    ...
    $w['AgamaID'] = 'K';
    $w['NA'] = 'N';
    $Homebase = AmbilCombo2('prodi', "concat(ProdiID, ' - ', Nama)", 'ProdiID', '', '', 'ProdiID');
  ...
  $optprodi = AmbilCekBox("prodi", 
	"ProdiID",
    "concat(ProdiID, ' - ', Nama) as NM", 
	"NM", 
	$w['ProdiID'], 
	'.');
  $TglLahir = AmbilComboTgl($w['TanggalLahir'], 'TglLahir');
  $optagm = AmbilCombo2('agama', "concat(Agama, ' - ', Nama)", 'Agama', $w['AgamaID'], '', 'Agama');
  $radkel = AmbilRadio("select Nama, Kelamin from kelamin order by Nama", "Kelamin", "Nama", "Kelamin", $w['KelaminID'], ", ");    
  ...
  <tr><td colspan=2 align=center>
    ...
    <input type=button name='Batal' value='Batal' onClick=\"location='?ndelox=$ndelox&gos=&$snm=$sid'\"></td></tr>
  </form></table></p>";
}
...
function DsnAddSav($gos='DsnEdt') {
  ...
  if ($md == 0) {
    $s = "update dosen set NIDN='$NIDN', Nama='$Nama',
      ...
      where Login='$Login' ";
    $r = mysqli_query($koneksi, $s);
    SuksesTersimpan("?ndelox=$_SESSION[ndelox]&gos=$gos&dsnid=$Login", 100);
  }
  else {
    $ada = AmbilFieldx('dosen', "Login", $Login, '*');
    if (empty($ada)) {
    ...
    else echo PesanError("Gagal", "Data dosen tidak dapat disimpan karena NIP: <b>$Login</b> sudah dipakai oleh:
      <b>$ada[Nama]</b>.<br>
      Gunakan NIP lain.<hr size=1 color=silver>
      Pilihan: <a href='?ndelox=$_SESSION[ndelox]&gos=DosenTambah&md=1'>Tambah Dosen</a> |
      <a href='?ndelox=$_SESSION[ndelox]&gos='>Kembali ke Daftar Dosen</a>");
  }
}
// *** Edit Dosen ***
function TampilkanHeaderDosenEdit($w) {
  ...
  <tr><td class=inp>Pilihan:</td>
      ...
      <input type=button name='Kembali' value='Kembali ke Daftar Dosen'
        onClick=\"location='?ndelox=$_SESSION[ndelox]&gos=&sub='\" />
      <input type=button name='Refresh' value='Refresh'
        onClick=\"location='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=0&dsnid=$_SESSION[dsnid]'\" />
      </td></tr>
  </table></p>";
}
function TampilkanMenuEditDosen($w) {
  ...
  for ($i = 0; $i < sizeof($arrMenuDosen); $i++) {
    ...
    echo "<td $c><a href='?ndelox=$_SESSION[ndelox]&gos=$mn[1]&dsnid=$w[Login]&dsnsub=$mn[2]'>$mn[0]</a></td>";
  }
  echo "</tr></table></p>";
}
function DsnEdt() {
  $w = AmbilFieldx('dosen', "Login", $_SESSION['dsnid'], '*');
...
function DsnEdtPribadi() {
  $_REQUEST['md']+0;
  DosenTambah($_SESSION['ndelox'], 'DsnEdtPribadiSav', 'DsnEdtPribadi');
}
...
function DsnEdtAkademik() {
  ...
  $w = AmbilFieldx('dosen', 'Login', $_SESSION['dsnid'], '*');
  $TglBekerja = AmbilComboTgl($w['TglBekerja'], 'TglBekerja');
  $StatusDosen = AmbilCombo2('statusdosen', "concat(StatusDosenID, ' - ', Nama)", 'StatusDosenID', $w['StatusDosenID'], '', 'StatusDosenID');
  $StatusKerja = AmbilCombo2('statuskerja', "concat(StatusKerjaID, ' - ', Nama)", 'StatusKerjaID', $w['StatusKerjaID'], '', 'StatusKerjaID');
  $Homebase = AmbilCombo2('prodi', "concat(ProdiID, ' - ', Nama)", 'ProdiID', $w['Homebase'], '', 'ProdiID');
  $_ProdiID = AmbilRadioProdi($w['ProdiID'], 'ProdiID');
  $Jenjang = AmbilCombo2('jenjang', "concat(JenjangID, ' - ', Nama)", 'JenjangID', $w['JenjangID'], '', 'JenjangID');
...
function DsnEdtJabatan() {
  ...
  $w = AmbilFieldx('dosen', 'Login', $_SESSION['dsnid'], '*');
  $Jabatan = AmbilCombo2('jabatan', "concat(JabatanID, ' - ', Nama)", 'JabatanID', $w['JabatanID'], '', 'JabatanID');
  $JabatanDikti = AmbilCombo2('jabatandikti', "concat(JabatanDiktiID, ' - ', Nama)", 'JabatanDiktiID', $w['JabatanDiktiID'], '', 'JabatanDiktiID');
  //$Golongan = AmbilCombo2('golongan', "concat(GolonganID, ' - ', KategoriID, ' - ', Pangkat)", 'GolonganID, KategoriID', $w['GolonganID'], "ProdiID='$w[Homebase]'", "concat(GolonganID,'~',KategoriID)");
  $optgol = GetOptGol($w['Homebase'], $w['GolonganID'], $w['KategoriID']);
  $optikt = AmbilCombo2("ikatan", "concat(IkatanID, ' - ', Nama, ' (', format(Besar, 0), ')')", "IkatanID", $w['IkatanID'], '', 'IkatanID');
  ...
  <tr><td class=inp>Jabatan Akademik</td>
      <td class=ul><select name='JabatanID'>$Jabatan</select> <a href='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=1&dsnsub=JbtnEdt'>Tambah  </a></td></tr>
  ...
  <tr><td class=inp>Golongan</td>
      <td class=ul><select name='GolonganID'>$optgol</select> <a href='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=1&dsnsub=GolEdt'>Tambah  </a></td></tr>
  <tr><td class=inp>Tunjangan Ikatan</td>
      <td class=ul><select name='IkatanID'>$optikt</select> <a href='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=1&dsnsub=IktEdt'>Tambah  </a></td></tr>
...
function DsnEdtAkademikSav() {
  ...
  $s = "update dosen
    ...
    where Login='$dsnid' ";
  $r = mysqli_query($koneksi, $s);
  SuksesTersimpan("?ndelox=$_SESSION[ndelox]&gos=DsnEdt&dsnid=$dsnid&dsnsub=DsnEdtAkademik", 100);
}
function DsnEdtJabatanSav() {
  ...
  $s = "update dosen set JabatanID='$JabatanID', JabatanDiktiID='$JabatanDiktiID',
    ...
    where Login='$dsnid' ";
  $r = mysqli_query($koneksi, $s);
  SuksesTersimpan("?ndelox=$_SESSION[ndelox]&gos=DsnEdt&dsnid=$dsnid&dsnsub=DsnEdtJabatan", 100);
}
function DsnAlmt() {
  $w = AmbilFieldx('dosen', 'Login', $_SESSION['dsnid'], '*');
  echo "<p><table class=box cellspacing=1 cellpadding=4 width=600>
...
function DsnAlmtSav() {
  ...
  $s = "update dosen set KTP='$KTP', Telephone='$Telephone',
    ...
    where Login='$dsnid' ";
  $r = mysqli_query($koneksi, $s);
  SuksesTersimpan("?dsnid=$dsnid&ndelox=$_SESSION[ndelox]&gos=dsnedt&dsnsub=DsnAlmt", 100);
}
...
function DsnEdtPendidikan() {
  $md = $_REQUEST['md']+0;
  if($md == 0){
  	$w_ = AmbilFieldx('dosenpendidikan', 'DosenPendidikanID', $_REQUEST['DosenPendidikanID'], '*');
	  $PT = AmbilFieldx('perguruantinggi','PerguruanTinggiID',$w_['PerguruanTinggiID'],'Nama,PerguruanTinggiID');
  ...
  $w = AmbilFieldx('dosen','Login',$_SESSION['dsnid'],'*');
  $Tglijazah = AmbilComboTgl($w_['TanggalIjazah'], 'Tglijazah');
  $Jenjang = AmbilCombo2('jenjang', "concat(JenjangID, ' - ', Nama)", 'JenjangID', $w_['JenjangID'], '', 'JenjangID');
  //$Nomor = AmbilCombo2('jenjang','JenjangID','JenjangID',$w_['Nomor'],'','JenjangID');
  $NoBenua = AmbilCombo2('benua',"concat(KodeBenua,' - ', NamaBenua)",'KodeBenua',$w_['KodeBenua'],'','KodeBenua');
  $Negara = AmbilCombo2('negara','NamaNegara','NamaNegara',$w_['NamaNegara'],'','NamaNegara');
  ...
  <tr><td colspan=2><input type=submit name='submit' value='Simpan'>
    <input type=reset name='Reset' value='Reset'> 
	<input type=button name='batal' value='Batal' onClick=\"location='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&dsnid=1001&dsnsub=TampilPendidikan'\"></td></tr>
  </form></table>";
...
function TampilKerjaDsn1(){
  ...
  $lst->pageactive = "=PAGE=";
  $lst->pages = "<a href='?ndelox=$_SESSION[ndelox]&gos=&dsnpage==PAGE='>=PAGE=</a>";
  $lst->tables = "dosenpekerjaan
    where DosenID='$_SESSION[dsnid]' $where
    order by DosenPekerjaanID";
  //$NamaPT = AmbilOneField('perguruantinggi','PerguruanTinggiID','=PerguruanTinggiID=','Nama');
  $lst->fields = "* ";
  $lst->headerfmt = "<p><table class=box cellspacing=1 cellpadding=4 width=800>
    <tr>
      <td class=ul colspan=9>
      <a href='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=1&dsnid=$_SESSION[dsnid]&dsnsubs=TampilKerjaDsn&dsnsub=DsnEdtPekerjaan'>Tambah Pekerjaan</td>
      </tr>
    <tr>
	  <th class=ttl>#</th>
  ...
  $lst->detailfmt = "<tr>
      ...
      <td class=cna=NA=><a href=\"?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=0&dpid==DosenPekerjaanID=&dsnid=$_SESSION[dsnid]&dsnsub=TampilKerjaDsn&dsnsub1=DsnEdtPekerjaan\"><img src='img/edit.png' border=0>
      </a></td>
...
function TampilTelitiDsn1(){
  ...
  $lst->headerfmt = "<p><table class=box cellspacing=1 cellpadding=4 width=600>
    <td class=ul colspan=9><a href='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=1&dsnid=$_SESSION[dsnid]&dsnsub=TampilTelitiDsn&dsnsub1=DsnEdtPenelitian'>Tambah Penelitian</td></tr>
    <tr>
  ...
  $lst->detailfmt = "<tr>
    ...
    <td class=cna=NA= width=10>
      <a href=\"?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=0&dlid==DosenPenelitianID=&dsnid=$_SESSION[dsnid]&dsnsub=TampilTelitiDsn&dsnsub1=DsnEdtPenelitian\"><img src='img/edit.png' border=0>
    </a></td>
...
function DsnEdtPenelitian() {
  $md = $_REQUEST['md']+0;
  if($md == 0){
  	$w = AmbilFieldx('dosenpenelitian', 'DosenPenelitianID', $_REQUEST['dlid'], '*');
  ...
  //$w  = AmbilFieldx('dosen','DosenID',$_SESSION['dsnid'],'*');
  //$Kota = AmbilCombo2('perguruantinggi', 'Kota', 'Kota', $w['Kota'], '', 'Kota');
  ...
  <tr><td colspan=2><input type=submit name='submit' value='Simpan'>
    <input type=reset name='Reset' value='Reset'> 
	<input type=button name='batal' value='Batal' onClick=\"location='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&dsnid=$_SESSION[dsnid]&dsnsub=TampilTelitiDsn'\"></td></tr>
  </form></table>";
}
function DsnEdtPenelitianSav() {
  ...
  SuksesTersimpan("?ndelox=$_SESSION[ndelox]&gos=DsnEdt&dsnid=$dsnid&dsnsub=TampilTelitiDsn", 100);
}

function JbtnEdt() {
  $md = $_REQUEST['md']+0;
  if($md == 0){
  	$w = AmbilFieldx('jabatan', 'JabatanID', $_REQUEST['jbtid'], '*');
	  $jdl = "Jabatan";   
	  $dis='disabled';
  }
  else {
...
<tr><td class=inp1>NA?</td><td class=ul><input type=checkbox name='na' value='Y' $c></td></tr>
  ...
  <tr><td colspan=2><input type=submit name='submit' value='Simpan'>
    <input type=reset name='Reset' value='Reset'> 
	<input type=button name='batal' value='Batal' onClick=\"location='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&dsnid=$_SESSION[dsnid]&dsnsub=DsnEdtJabatan'\"></td></tr>
  </form></table>";
  
  DaftarJabatan($_SESSION['ndelox'], "gos=DsnEdt&md=0&jbtid==JabatanID=", "Nama");
...
function JbtnEdtSav($gos='DsnEdt') {
  ...
  if ($md == 0) {
    ...
    $r = mysqli_query($koneksi, $s);
    SuksesTersimpan("?ndelox=$_SESSION[ndelox]&gos=$gos&md=1&dsnsub=JbtnEdt", 100);
  }
  else {
    $ada = AmbilFieldx('jabatan', "JabatanID", $JabatanID, '*');
    if (empty($ada)) {
    ...
    else echo PesanError("Gagal", "Data Jabatan tidak dapat disimpan karena Kode Jabatan: <b>$JabatanID</b> sudah dipakai oleh:
      <b>$ada[Nama]</b>.<br>
      Gunakan Kode lain.<hr size=1 color=silver>
      Pilihan: <a href='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=1&dsnsub=JbtnEdt'>Tambah Jabatan</a>");
  }
}
function GolEdt() {
  $md = $_REQUEST['md']+0;
  if($md == 0){
  	$w = AmbilFieldx('golongan', 'GolonganID', $_REQUEST['golid'], '*');
	  $jdl = "Golongan";   
	  $dis='disabled';
  }
  else {
...
<td class=inp1>NA?</td><td class=ul><input type=checkbox name='na' value='Y' $c></td></tr>
  ...
  DaftarGolongan($_SESSION['ndelox'], "gos=DsnEdt&md=0&golid==GolonganID=", "KategoriID,ProdiID,Pangkat,Nama,TunjanganFungsional,TunjanganSKS,TunjanganTransport,TunjanganTetap");
...
function GolEdtSav($gos='DsnEdt') {
  ...
  if ($md == 0) {
    ...
    $r = mysqli_query($koneksi, $s);
    SuksesTersimpan("?ndelox=$_SESSION[ndelox]&gos=$gos&md=1&dsnsub=GolEdt", 100);
  }
  else {
    $ada = AmbilFieldx('golongan', "GolonganID", $GolonganID, '*');
    if (empty($ada)) {
    ...
    else echo PesanError("Gagal", "Data Golongan tidak dapat disimpan karena Kode Golongan: <b>$GolonganID</b> sudah dipakai oleh:
      <b>$ada[Pangkat]</b>.<br>
      Gunakan Kode lain.<hr size=1 color=silver>
      Pilihan: <a href='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=1&dsnsub=GolEdt'>Tambah Golongan</a>");
  }
}
...
function IktEdt() {
  $md = $_REQUEST['md']+0;
  if($md == 0){
  	$w = AmbilFieldx('ikatan', 'IkatanID', $_REQUEST['iktid'], '*');
	  $jdl = "Ikatan";   
	  $dis='disabled';
  }
  else {
...
<tr><td class=inp1>NA?</td><td class=ul><input type=checkbox name='na' value='Y' $c></td></tr>
  ...
  DaftarIkatan($_SESSION['ndelox'], "gos=DsnEdt&md=0&iktid==IkatanID=", "Nama,Besar");
...
function IktEdtSav($gos='DsnEdt') {
  ...
  if ($md == 0) {
    ...
    $r = mysqli_query($koneksi, $s);
    SuksesTersimpan("?ndelox=$_SESSION[ndelox]&gos=$gos&md=1&dsnsub=IktEdt", 100);
  }
  else {
    $ada = AmbilFieldx('ikatan', "IkatanID", $IkatanID, '*');
    if (empty($ada)) {
    ...
    else echo PesanError("Gagal", "Data Ikatan tidak dapat disimpan karena Kode Ikatan: <b>$IkatanID</b> sudah dipakai oleh:
      <b>$ada[Nama]</b>.<br>
      Gunakan Kode lain.<hr size=1 color=silver>
      Pilihan: <a href='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=1&dsnsub=IktEdt'>Tambah Ikatan</a>");
  }
}
...
function TampilPendidikan1(){
  ...
  $lst->headerfmt = "<p><table class=box cellspacing=1 cellpadding=4>
  	<td class=ul colspan=9><a href='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=1&dsnid=$_SESSION[dsnid]&dsnsub=TampilPendidikan&dsnsub1=DsnEdtPendidikan'>Tambah Pendidikan</td></tr>
    <tr>
	  <th class=ttl>#</th>
	  <th class=ttl>Edit</th>
  ...
  $lst->detailfmt = "<tr>
      ...
      <td class=cna=NA=><a href=\"?ndelox=$_SESSION[ndelox]&gos=DsnEdt&md=0&dsnid=$_SESSION[dsnid]&dsnsub=TampilPendidikan&dsnsub1=DsnEdtPendidikan&DosenPendidikanID==DosenPendidikanID=\"><img src='img/edit.png' border=0>
      </a></td>
...
function DsnEdtPekerjaan() {
  $md = $_REQUEST['md']+0;
  if($md == 0){
    $w = AmbilFieldx('dosenpekerjaan', 'DosenPekerjaanID', $_REQUEST['dpid'], '*');
  ...
  <tr><td colspan=2><input type=submit name='submit' value='Simpan'>
    <input type=reset name='Reset' value='Reset'> 
	<input type=button name='batal' value='Batal' onClick=\"location='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&dsnid=$_SESSION[dsnid]&dsnsub=TampilKerjaDsn'\"></td></tr>
  </form></table>";
}
...
function DsnEdtPekerjaanSav() {
  ...
  SuksesTersimpan("?ndelox=$_SESSION[ndelox]&gos=DsnEdt&dsnid=$dsnid&dsnsub=TampilKerjaDsn", 100);
}
function DsnEdtPengajaran() {
  $dsnajarpage = GainVariabelx('dsnajarpage', 1);
  $lst = new dwolister;
  $lst->maxrow = 20;
  $lst->page = $_SESSION['dsnajarpage']+0;
  $lst->pageactive = "=PAGE=";
  $lst->pages = "<a href='?ndelox=$_SESSION[ndelox]&gos=DsnEdt&dsnajarpage==PAGE='>=PAGE=</a>";
  $lst->tables = "jadwal j
...
$dsnsub = GainVariabelx('dsnsub');
$dsnurt = GainVariabelx('dsnurt', 'Login');
$dsnid = GainVariabelx('dsnid');
$dsncr = GainVariabelx('dsncr');
$dsnkeycr = GainVariabelx('dsnkeycr');
$dsnpage = GainVariabelx('dsnpage');
$prodi = GainVariabelx('prodi');
$last = GainVariabelx('last', "H");
if ($dsnkeycr == 'Reset') {
  $dsncr = '';
  $_SESSION['dsncr'] = '';
  $dsnkeycr = '';
