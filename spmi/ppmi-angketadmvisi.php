...
<h3 class="box-title"><b style='color:green;font-size:20px'>Angket Pemahaman Visi Misi serta Tujuan STMIK HTP</b></h3>  
<form style='margin-right:5px; margin-top:0px' class='pull-right' action='' method='GET'>
<input type="hidden" name='ndelox' value='spmi/ppmi-angketadmvisi'>
...
$tampil = mysqli_query($koneksi, "SELECT * from t_ppmiangketvisi Group by RespondenKategori"); 									
while($r=mysqli_fetch_array($tampil)){ 
$responden = mysqli_fetch_array(mysqli_query($koneksi, "SELECT count(*) as jml from t_ppmiangketvisi"));   					         
$no++;
...
<a href='index.php?ndelox=spmi/ppmi-angketadmvisi&act=viewangket&tahun=$r[TahunID]&prodi=$_GET[prodi]&JadwalID=$r[JadwalID]'> View Data ($responden[jml] Orang responden)</a>
| <a href='print_reportxls/angketvisixls.php?tahun=$r[TahunID]&prodi=$_GET[prodi]&JadwalID=$r[JadwalID]&DosenID=$r[DosenID]&MKID=$r[MKID]' target=_BLANK> Exp XLS</a>
</center>
</td>
</tr>";
 }
...
if (isset($_GET['hapus'])){
    ...
    echo "<script>document.location='index.php?ndelox=spmi/ppmi-angketadmvisi&JadwalID=$_GET[JadwalID]&tahun=$_GET[tahun]&prodi=$_GET[prodi]&sukses';</script>";
}
...
for($i = 1; $i <= $JumData; $i++){
    ...
    $Jawaban  		= strfilter($_POST['Jawaban'.$i]);	
		if (!empty($PertanyaanID)){     
			...
			if (mysqli_num_rows($sqlcek)>0){
				...
				window.location = 'index.php?ndelox=spmi/ppmi-angketadmvisi&JadwalID=$_POST[JadwalID]&tahun=$_POST[tahun]&prodi=$_POST[prodi]&gagal'</script>";
				exit;
			}
	...
	if ($query){
		echo "<script>document.location='index.php?ndelox=spmi/ppmi-angketadmvisi&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[_Login]&sukses';</script>";                       
	}else{
		echo "<script>document.location='index.php?ndelox=spmi/ppmi-angketadmvisi&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[_Login]&gagal';</script>";
	}
...
$sq = mysqli_query($koneksi, "SELECT distinct(MhswID) from t_ppmiangketvisi where TahunID='".strfilter($_GET['tahun'])."'"); 
