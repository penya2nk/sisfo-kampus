...
<div class="col-xs-12">  
  <div class="box">
	<div class="box-header">
	  <h3 class="box-title"><b style='color:green;font-size:20px'>Manajemen Dokumen SOP Pengabdian</b> 					
	</div><!-- /.box-header -->
	<div class="box-body">			   
	   <a class='pull-right btn btn-primary btn-sm' href='index.php?ndelox=spmi/spmiadmppmisoppkm&act=tambah&tahun=<?php echo $_GET[tahun]; ?>&prodi=<?php echo "$_GET[prodi]";?>'>Tambah Dokumen</a>
	  ...
	  <?php
		...
		$sql = mysqli_query($koneksi, "SELECT * from t_ppmisop where KategoriSOPID='PKM'");
		$no=1;
...
echo "<a style='margin-right:5px; width:106px' class='btn btn-info btn-xs' title='View' href='$r[Link]' target=_blank><i class='fa fa-download'></i></span> View </a>";				  
				  echo "<a class='btn btn-success btn-xs' title='Edit' href='index.php?ndelox=spmi/spmiadmppmisoppkm&act=edit&prodi=$r[ProdiID]&id=$_GET[id]&tahun=$r[TahunID]&edit=$r[DokID]'><i class='fa fa-edit'></i></a>
							<a class='btn btn-danger btn-xs' title='Delete' href='index.php?ndelox=spmi/spmiadmppmisoppkm&prodi=$r[ProdiID]&tahun=$r[TahunID]&hapus=$r[DokID]' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><i class='fa fa-trash'></i></a>
				  </td>
				</tr>";
	...
	<?php 
		...
		if (isset($_GET['hapus'])){
			...
			echo "<script>document.location='index.php?ndelox=spmi/spmiadmppmisoppkm&prodi=".$_GET['prodi']."&id=".$_GET['id']."&tahun=".$_GET['tahun']."'</script>";
		  }
	?>
	</div>
</div>
...
if (isset($_POST['tambah'])){
        ...
        echo "<script>document.location='index.php?ndelox=spmi/spmiadmppmisoppkm&prodi=".$_POST['prodi']."&id=".$_GET['id']."&tahun=".$_POST['tahun']."';</script>";
      }
...
echo "<div class='col-md-12'>
		  ...
		  <div class='box-footer'>
				...
				<a href='index.php?ndelox=spmi/spmiadmppmisoppkm&tahun=$_GET[tahun]&prodi=$_GET[prodi]'><button type='button' class='btn btn-default pull-right'>Cancel</button></a>
