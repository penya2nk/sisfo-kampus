...
<h3 class="box-title"><b style='color:green;font-size:20px'>Angket Kepuasan Layanan Akademik STMIK HTP</b></h3>  
<form style='margin-right:5px; margin-top:0px' class='pull-right' action='' method='GET'>
<input type="hidden" name='ndelox' value='spmi/ppmi-angketadmlayanan'>
...
$responden = mysqli_fetch_array(mysqli_query($koneksi, "SELECT count(*) as jml from t_ppmipertanyaanlayanan"));   					         
$no++;
...
<a href='index.php?ndelox=spmi/ppmi-angketadmlayanan&act=viewangket&tahun=$r[TahunID]&prodi=$_GET[prodi]&JadwalID=$r[JadwalID]'> View Data ($responden[jml] Orang responden)</a>
| <a href='print_reportxls/angketlayananxls.php?tahun=$r[TahunID]&prodi=$_GET[prodi]&JadwalID=$r[JadwalID]&DosenID=$r[DosenID]&MKID=$r[MKID]' target=_BLANK> Exp XLS</a>
</center>
</td>
</tr>";
 }
...
for($i = 1; $i <= $JumData; $i++){
    ...
    $Jawaban  		= strfilter($_POST['Jawaban'.$i]);	
		if (!empty($PertanyaanID)){     
			...
			if (mysqli_num_rows($sqlcek)>0){
				...
				window.location = 'index.php?ndelox=spmi/ppmi-angketadmlayanan&JadwalID=$_POST[JadwalID]&tahun=$_POST[tahun]&prodi=$_POST[prodi]&gagal'</script>";
				exit;
			}
	...
	if ($query){
		echo "<script>document.location='index.php?ndelox=spmi/ppmi-angketadmlayanan&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&sukses';</script>";                       
	}else{
		echo "<script>document.location='index.php?ndelox=spmi/ppmi-angketadmlayanan&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&gagal';</script>";
	}
...
$sq = mysqli_query($koneksi, "SELECT distinct(MhswID) from t_ppmiangketlayanan where TahunID='".strfilter($_GET['tahun'])."'"); 
