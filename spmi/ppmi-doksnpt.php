...
<div class="col-xs-12">  
  <div class="box">
	...
	<div class="box-body">			   
	   <a class='pull-right btn btn-primary btn-sm' href='index.php?ndelox=spmi/spmiadmppmidoksnpt&act=tambah&tahun=<?php echo $_GET[tahun]; ?>&prodi=<?php echo "$_GET[prodi]";?>'>Tambah Dokumen</a>
	  ...
	  <?php
		...
		echo "<tr><td colspan=5 style=text-align:left;font-size:16px;color:#FFFFFF;font-weight:reguler;background-color:#e62899;>$k[Nama]</td>";
...
echo "<a style='margin-right:5px; width:106px' class='btn btn-info btn-xs' title='View' href='$r[Link]' target=_blank><i class='fa fa-download'></i></span> Lihat Dok </a>";
				  echo "<a class='btn btn-success btn-xs' title='Edit' href='index.php?ndelox=spmi/spmiadmppmidoksnpt&act=edit&prodi=$r[ProdiID]&id=$_GET[id]&tahun=$r[TahunID]&edit=$r[DokID]'><i class='fa fa-edit'></i></a>
							<a class='btn btn-danger btn-xs' title='Delete' href='index.php?ndelox=spmi/spmiadmppmidoksnpt&prodi=$r[ProdiID]&tahun=$r[TahunID]&hapus=$r[DokID]' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><i class='fa fa-trash'></i></a>
				  
				  </td>
				</tr>";
				//}elseif ($_SESSION[level]=='120'){
                /*  if ($_SESSION[level]=='120'){
	...
	<?php 
		...
		if (isset($_GET['hapus'])){
			...
			echo "<script>document.location='index.php?ndelox=spmi/spmiadmppmidoksnpt&prodi=".$_GET['prodi']."&id=".$_GET['id']."&tahun=".$_GET['tahun']."'</script>";
		  }
	?>
	</div>
</div>
...
cek_session_admppmi();
        mysqli_query($koneksi, "INSERT INTO t_ppmidoksnpt(KategoriSNPTID,nama_file,TanggalBuat,Revisi,Keterangan) 
        ...
        echo "<script>document.location='index.php?ndelox=spmi/spmiadmppmidoksnpt&prodi=".$_POST['prodi']."&id=".$_GET['id']."&tahun=".$_POST['tahun']."';</script>";
      }
...
echo "<div class='col-md-12'>
		  ...
		  <div class='box-footer'>
				...
				<a href='index.php?ndelox=spmi/spmiadmppmidoksnpt&tahun=$_GET[tahun]&prodi=$_GET[prodi]'><button type='button' class='btn btn-default pull-right'>Cancel</button></a>
...
if (isset($_POST['update'])){
        mysqli_query("UPDATE t_ppmidoksnpt SET 
