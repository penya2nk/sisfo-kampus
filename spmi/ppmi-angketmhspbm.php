...
<input type="hidden" name='ndelox' value='mhsangketpbm'>
...
$jjwb   =mysqli_fetch_array(mysqli_query($koneksi, "select Count(PertanyaanID) as Jml from t_ppmiangket where JadwalID='$r[JadwalID]' AND TahunID='$_SESSION[tahun_akademik]' AND MhswID='$_SESSION[_Login]'"));			         
if ($jjwb['Jml']>0){
	$c="style=color:red";
}else{
	$c="style=color:green";
...
<td>";
if ($jjwb['Jml']>0){
	echo"| <a href='index.php?ndelox=spmi/spmimhsangketpbm&act=viewangket&JadwalID=$jdwl[JadwalID]&tahun=$jdwl[TahunID]&MKID=$jdwl[MKID]&DosenID=$jdwl[DosenID]&prodi=$mk[ProdiID]'> Lihat Angket </a> <b style=color:green>($jjwb[Jml])</b> |</a>";
}else{		
	echo"<a href='index.php?ndelox=spmi/spmimhsangketpbm&act=tambahdata&JadwalID=$jdwl[JadwalID]&tahun=$jdwl[TahunID]&MKID=$jdwl[MKID]&DosenID=$jdwl[DosenID]&prodi=$mk[ProdiID]'> | Isi Angket</a>";		
}
echo"</td>
</tr>";
 }
...
if (isset($_GET['hapus'])){
    ...
    echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketpbm&JadwalID=$_GET[JadwalID]&tahun=$_GET[tahun]&prodi=$_GET[prodi]&sukses';</script>";
}
...
<a href='index.php?ndelox=spmi/spmimhsangketpbm&tahun=$_GET[tahun]&prodi=$_GET[prodi]'><button type='button' class='btn btn-default pull-right'>Back</button></a>                    
...
if (isset($_POST['simpann'])){
    ...
    $Jawaban  		= strfilter($_POST['Jawaban'.$i]);	
		if (!empty($PertanyaanID)){     
			...
			if (mysqli_num_rows($sqlcek)>0){
				...
				window.location = 'index.php?ndelox=spmi/spmimhsangketpbm&JadwalID=$_POST[JadwalID]&tahun=$_POST[tahun]&prodi=$_POST[prodi]&gagal'</script>";
				exit;
			}
	...
	if ($query){
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketpbm&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[_Login]&sukses';</script>";                       
	}else{
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketpbm&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[_Login]&gagal';</script>";
	}
...
<a href='index.php?ndelox=spmi/spmimhsangketpbm&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[_Login]&sukses''><button type='button' class='btn btn-default pull-right'>Kembali Ke Soal</button></a>                    
</div>";
echo "</form>";
...
for($i = 1; $i <= $JumData; $i++){
    ...
    $Jawaban  		= strfilter($_POST['Jawaban'.$i]);	
		if (!empty($PertanyaanID)){     
			$sqlcek = mysqli_query($koneksi, "SELECT * FROM t_ppmiangket WHERE MhswID='$MhswID' and JadwalID='$_POST[JadwalID]'");
			if (mysqli_num_rows($sqlcek)>0){
	...
	if ($query){
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketpbm&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&sukses';</script>";                       
	}else{
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketpbm&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&gagal';</script>";
	}
