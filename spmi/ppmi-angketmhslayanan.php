...
<input type="hidden" name='ndelox' value='mhsangketlayanan'>
...
<tr><td colspan=2>
<a href='index.php?ndelox=spmi/spmimhsangketlayanan&act=tambahdata&JadwalID=$r[JadwalID]&tahun=$_SESSION[tahun_akademik]&MKID=$r[MKID]&DosenID=$r[DosenID]&prodi=$r[ProdiID]'> | Klik Link Pengisian Angket</a>
| <a href='index.php?ndelox=spmi/spmimhsangketlayanan&act=viewangket&JadwalID=$r[JadwalID]&tahun=$_SESSION[tahun_akademik]&MKID=$r[MKID]&DosenID=$r[DosenID]&prodi=$r[ProdiID]'> Lihat Angket</a>
</td></tr>
...
<a href='index.php?ndelox=spmi/spmimhsangketlayanan&tahun=$_GET[tahun]&prodi=$_GET[prodi]'><button type='button' class='btn btn-default pull-right'>Back</button></a> 
...
for($i = 1; $i <= $JumData; $i++){
    ...
    $Jawaban  		= $_POST['Jawaban'.$i];	
		if (!empty($PertanyaanID)){     
			...
			if (mysqli_num_rows($sqlcek)>0){
				...
				window.location = 'index.php?ndelox=spmi/spmimhsangketlayanan&JadwalID=$_POST[JadwalID]&tahun=$_POST[tahun]&prodi=$_POST[prodi]&gagal'</script>";
				exit;
			}
	...
	if ($query){
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketlayanan&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[_Login]&sukses';</script>";                       
	}else{
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketlayanan&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[_Login]&gagal';</script>";
	}
...
for($i = 1; $i <= $JumData; $i++){
    ...
    $Jawaban  		= strfilter($_POST['Jawaban'.$i]);	
		if (!empty($PertanyaanID)){     
			$sqlcek = mysqli_query($koneksi, "SELECT * FROM t_ppmiangketlayanan WHERE MhswID='$MhswID' and TahunID='$_SESSION[tahun_akademik]'");
			if (mysqli_num_rows($sqlcek)>0){
	...
	if ($query){
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketlayanan&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&sukses';</script>";                       
	}else{
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketlayanan&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&gagal';</script>";
	}
