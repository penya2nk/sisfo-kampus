...
<input type="hidden" name='ndelox' value='mhsangketpjj'>
...
<tr><td colspan=2>
<a href='index.php?ndelox=spmi/spmimhsangketpjj&act=tambahdata&JadwalID=$r[JadwalID]&tahun=$r[TahunID]&MKID=$r[MKID]&DosenID=$r[DosenID]&prodi=$r[ProdiID]'> | Klik Link Pengisian Angket</a>
| <a href='index.php?ndelox=spmi/spmimhsangketpjj&act=viewangket&JadwalID=$r[JadwalID]&tahun=$r[TahunID]&MKID=$r[MKID]&DosenID=$r[DosenID]&prodi=$r[ProdiID]'> Lihat Angket</a>
</td></tr>
...
<a href='index.php?ndelox=spmi/spmimhsangketpjj&tahun=$_GET[tahun]&prodi=$_GET[prodi]'><button type='button' class='btn btn-default pull-right'>Back</button></a> 
...
for($i = 1; $i <= $JumData; $i++){
    ...
    $Jawaban  		= strfilter($_POST['Jawaban'.$i]);	
		if (!empty($PertanyaanID)){     
			...
			if (mysqli_num_rows($sqlcek)>0){
				...
				window.location = 'index.php?ndelox=spmi/spmimhsangketpjj&JadwalID=$_POST[JadwalID]&tahun=$_POST[tahun]&prodi=$_POST[prodi]&gagal'</script>";
				exit;
			}
		$query = mysqli_query("INSERT INTO t_ppmiangketpjj 
						   (JadwalID,
						    MhswID,
							PertanyaanID,
							Jawaban,
	...
	if ($query){
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketpjj&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&sukses';</script>";                       
	}else{
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketpjj&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&gagal';</script>";
	}
...
$angket = mysqli_fetch_array(mysqli_query($koneksi, "SELECT JadwalID,MhswID FROM t_ppmiangketpjj WHERE JadwalID='".strfilter($_GET['JadwalID'])."' and MhswID='".strfilter($_GET['MhswID'])."'"));
...
for($i = 1; $i <= $JumData; $i++){
    ...
    $Jawaban  		= strfilter($_POST['Jawaban'.$i]);	
		if (!empty($PertanyaanID)){     
			$sqlcek = mysqli_query($koneksi, "SELECT * FROM t_ppmiangketpjj WHERE MhswID='$MhswID' and JadwalID='".strfilter($_POST['JadwalID'])."'");
			if (mysqli_num_rows($sqlcek)>0){
	...
	if ($query){
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketpjj&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[_Login]&sukses';</script>";                       
	}else{
		echo "<script>document.location='index.php?ndelox=spmi/spmimhsangketpjj&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[_Login]&gagal';</script>";
	}
