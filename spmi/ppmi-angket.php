...
<input type="hidden" name='ndelox' value='spmi/ppmi-angket'>
...
<a href='index.php?ndelox=spmi/ppmi-angket&act=viewangket&tahun=$_GET[tahun]&prodi=$_GET[prodi]&JadwalID=$r[JadwalID]'> View Data ($responden[jml] Orang responden)</a>
...
if (isset($_GET['hapus'])){
    ...
    echo "<script>document.location='index.php?ndelox=spmi/ppmi-angket&JadwalID=$_GET[JadwalID]&tahun=$_GET[tahun]&prodi=$_GET[prodi]&sukses';</script>";
}
...
for($i = 1; $i <= $JumData; $i++){
    ...
    $Jawaban  		= strfilter($_POST['Jawaban'.$i]);	
		if (!empty($PertanyaanID)){     
			...
			if (mysqli_num_rows($sqlcek)>0){
				...
				window.location = 'index.php?ndelox=spmi/ppmi-angket&JadwalID=$_POST[JadwalID]&tahun=$_POST[tahun]&prodi=$_POST[prodi]&gagal'</script>";
				exit;
			}
	...
	if ($query){
		echo "<script>document.location='index.php?ndelox=spmi/ppmi-angket&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&sukses';</script>";                       
	}else{
		echo "<script>document.location='index.php?ndelox=spmi/ppmi-angket&act=viewangket&JadwalID=$_POST[JadwalID]&prodi=$_POST[prodi]&tahun=$_POST[tahun]&MhswID=$_SESSION[id]&gagal';</script>";
	}
...
//$angket = mysqli_fetch_array(mysqli_query($koneksi, "SELECT JadwalID,MhswID FROM t_ppmiangket WHERE JadwalID='$_GET[JadwalID]' "));//and MhswID='$_SESSION[id]'
