...
$_presensidoNama = GainVariabelx('_presensidoNama');
$_presensidoProdi = GainVariabelx('_presensidoProdi');
$_presensidoPrg = GainVariabelx('_presensidoPrg');
$_presensidoNomer = GainVariabelx('_presensidoNomer');
$_presensidoPage = GainVariabelx('_presensidoPage');
$_presensidoUrut = GainVariabelx('_presensidoUrut', 1);
$_presensidoTglMulai_y = GainVariabelx('_presensidoTglMulai_y', date('Y', strtotime( '-30 days')));
$_presensidoTglMulai_m = GainVariabelx('_presensidoTglMulai_m', date('m', strtotime( '-30 days')));
$_presensidoTglMulai_d = GainVariabelx('_presensidoTglMulai_d', date('d', strtotime( '-30 days')));
$_presensidoTglSelesai_y = GainVariabelx('_presensidoTglSelesai_y', date('Y'));
$_presensidoTglSelesai_m = GainVariabelx('_presensidoTglSelesai_m', date('m'));
$_presensidoTglSelesai_d = GainVariabelx('_presensidoTglSelesai_d', date('d'));
...
// *** Main ***
TitleApps("Pengurusan Drop Out Presensi");
  
  $lungo = (empty($_REQUEST['lungo']))? 'DropOutPresensi' : $_REQUEST['lungo'];
  $lungo();
...
function TampilkanHeaderFilter() {
  $optprg = AmbilCombo2('program', "concat(ProgramID, ' - ', Nama)", 'ProgramID', $_SESSION['_presensidoPrg'], "KodeID='".KodeID."'", 'ProgramID');
  $optprodi = AmbilCombo2('prodi', "concat(ProdiID, ' - ', Nama)", 'ProdiID', $_SESSION['_presensidoProdi'], "KodeID='".KodeID."'", 'ProdiID');
  $optstatusmhsw = AmbilCombo2('statusmhsw', "concat(StatusMhswID, ' - ', Nama)", 'StatusMhswID', $_SESSION['_presensidoID'], "", 'StatusMhswID');
  $opturut = AmbilUrutanMhswID();
  $opttglmulai = AmbilComboTgl("$_SESSION[_presensidoTglMulai_y]-$_SESSION[_presensidoTglMulai_m]-$_SESSION[_presensidoTglMulai_d]", '_presensidoTglMulai');
  $opttglselesai = AmbilComboTgl("$_SESSION[_presensidoTglSelesai_y]-$_SESSION[_presensidoTglSelesai_m]-$_SESSION[_presensidoTglSelesai_d]", '_presensidoTglSelesai');
  
  $NIMCheck = ($_SESSION['_presensidoNIMSementara'] == 'Y')? 'checked' : '';
  if(!empty($_SESSION['_presensidoProdi']))
	$TampilkanNIMSementara = (AmbilOneField('prodi', "ProdiID='$_SESSION[_presensidoProdi]' and KodeID", KodeID, 'GunakanNIMSementara') == 'Y')?
  ...
  <tr>
      ...
      <input type=button name='Reset' value='Reset'
        onClick=\"location='?ndelox=$_SESSION[ndelox]&lungo=&_presensidoPage=0&_presensidoNama=&_presensidoNomer='\" />
      </td>
  </form>
  </table>";
}
...
function DropOutPresensi() {
  ...
  {	$edit = "<a href='#' onClick=\"location='?ndelox=$_SESSION[ndelox]&lungo=StatusMhswEdt&md=0&id==ProsesStatusMhswID='\" />
  ...
  {	  $whr[] = "m.ProdiID = '$_SESSION[_presensidoProdi]' ";
	  ...
	  {	 $tmp = AmbilOneField('prodi', 'ProdiID', $_SESSION['_presensidoProdi'], 'FormatNIMSementara'); 
  ...
  $pagefmt = "<a href='?ndelox=$_SESSION[ndelox]&lungo=&_presensidoPage==PAGE='>=PAGE=</a>";
  $pageoff = "<b>=PAGE=</b>";
  ...
  //<td class=cna=NA= width=15 align=center><a href='#' onClick=\"PrintSurat('=MhswID=')\"><img title='Surat Keterangan' src='img/print.png'></a></td>
...
{	echo <<<SCR
	<script>
		...
		{	
			lnk = "$_SESSION[ndelox].printsurat.php?MhswID="+id;
		...
		{	if(confirm("Anda yakin akan menset status mahasiswa dengan id "+id+" menjadi Drop-Out?"))
			{	window.location='?ndelox=$_SESSION[ndelox]&lungo=DropMhsw&MhswID='+id;
			}
		}
		function PerincianKehadiran(id)
		{	window.location='?ndelox=$_SESSION[ndelox]&lungo=Perincian&MhswID='+id;
		}
	</script>
SCR;
}
...
$MhswID = $_REQUEST['MhswID'];
	$_MhswID = AmbilOneField('mhsw', "MhswID='$MhswID' and KodeID", KodeID, 'MhswID');
	if(empty($_MhswID))
...
global $koneksi;
$MhswID = $_REQUEST['MhswID'];
	$mhsw = AmbilFieldx('mhsw', "MhswID='$MhswID' and KodeID", KodeID, 'MhswID, Nama, ProdiID, StatusMhswID');
	$statusmhsw = AmbilFieldx('statusmhsw', 'StatusMhswID', $mhsw['StatusMhswID'], 'Nama');
	...
	echo "<tr><td colspan=6 align=center><input type=button name='Drop' value='Drop Mahasiswa ini' onClick=\"DropMhsw('$MhswID')\">
										 <input type=button name='Kembali' value='Kembali' onClick=\"location='?ndelox=$_SESSION[ndelox]&lungo='\" /></td></tr>
		</table>";
	DropOutScript();
}
?>
