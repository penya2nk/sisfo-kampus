...
$_remedialTahunID = GainVariabelx('_remedialTahunID');
$_remedialProdiID = GainVariabelx('_remedialProdiID');
...
$_remPage = GainVariabelx('_remPage', 0);

if ($_REQUEST['lungo'] != 'cetakLaporan'){
...
$lungo = (empty($_REQUEST['lungo']))? 'TampilkanHalaman' : $_REQUEST['lungo'];
$lungo();
...
{	
	TitleApps('Laporan Remedial Per Periode');
	$prodiopt = AmbilCombo2('prodi', "concat(ProdiID, ' - ', Nama)", 'ProdiID', $_SESSION['_remedialProdiID'], "KodeID='".KodeID."'", 'ProdiID');
	...
	echo "<table class=box cellspacing=1 align=center width=800>
			<form action='?' method=POST onSubmit=\"changePage(1, '$_SESSION[ndelox]')\">
...
{	
	...
	{	$counting++;
		...
		$r3 = mysqli_query($koneksi, $s3);
		$w3 = mysqli_fetch_array($r3);
