...
include_once "../header_pdf.php";
  ...
  $optstst = AmbilCombo2("statusmhsw", "concat(StatusMhswID, ' - ', Nama)", "StatusMhswID", $_SESSION['stts'], "StatusMhswID in ('A', 'P', 'C', 'T')", 'StatusMhswID');
    echo "<p><table class=box cellspacing=1 cellpadding=4>
  <form action='?' method=POST onSubmit=\"return CheckForm(this)\">
  <input type=hidden name='ndelox' value='akd.lap.jumlahsks'>
  <input type=hidden name='lungo' value='daftar'>
  ...
  $prodi = "";
	$first = 1;
  // Buat header
  /*$_prodi = AmbilOneField('prodi', 'ProdiID', $_SESSION['prodi'], 'Nama');
  $_prid = AmbilOneField('program', 'ProgramID', $_SESSION['prid'], 'Nama');
  ...
  while ($w = mysqli_fetch_array($r)) {
		...
		$_prodi = AmbilOneField('prodi', 'ProdiID', $w['ProdiID'], 'Nama');
    ...
    if (!empty($w['TotalSKS'])) {
      $n++; $brs++;
      $Status = AmbilOneField('statusmhsw', "StatusMhswID", $w['StatusMhswID'], 'Nama');
      $isi = str_pad($n.'.', 4, ' ') . ' ' .
        str_pad($w['MhswID'], 12) . ' '.
        str_pad($w['Nama'], 30) . ' '.
...
$prid = GainVariabelx('prid', 'REG');
$jmlsks = GainVariabelx('jmlsks', 140);
$stts = GainVariabelx('stts');
$angkatan = GainVariabelx('angkatan');
//echo $stts;
TitleApps("Daftar Jumlah SKS Mahasiswa");
