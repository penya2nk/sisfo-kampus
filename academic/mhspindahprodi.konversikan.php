...
ViewHeaderApps("Konversi Matakuliah");
...
// *** Main ***
$lungo = (empty($_REQUEST['lungo']))? "fnKonversikan" : $_REQUEST['lungo'];
$lungo($KRSID, $MhswID);
...
function fnKonversikan($KRSID, $MhswID) {
  ...
  <input type=hidden name='lungo' value='fnSimpan' />
...
// dirubah link carimkprodi nya
  var url = "../jur/carimk2prodi.php"
...
function fnSimpan($KRSID, $MhswID) {
  ...
  $nilai = AmbilFieldx('nilai', 'NilaiID', $NilaiID, '*');
  $KRSID = $_REQUEST['KRSID'];
  $oldkrs = AmbilFieldx('krs', 'KRSID', $KRSID, '*');
  
  $mhsw = AmbilFieldx('mhsw', "MhswID='$MhswID' and KodeID", KodeID, "*");
  $mk = AmbilFieldx('mk', 'MKID', $MKID, '*');
  
  // Ambil KHS
  $khs = AmbilFieldx('khs', "MhswID='$MhswID' and TahunID='$TahunID' and KodeID", KodeID, '*');
  if (empty($khs)) {
    $sesi = AmbilOneField('khs', "MhswID='$MhswID' and KodeID", KodeID, "max(Sesi)");
    $sesi += 1;
    $s = "insert into khs
      (TahunID, KodeID, 
