...
// *** Main ***
TitleApps("Mahasiswa Pindah Prodi");
...
function HeaderCariMhsw() {
  ...
  <tr><td class=inp width=100>NIM/NPM:</td>
      ...
      <td class=ul nowrap>
        ...
        <input type=button name='btnReset' value='Reset' onClick="location='?ndelox=$_SESSION[ndelox]&lungo=&crMhswID=&crNamaMhsw='" />
      </td>
  </tr>
  </form>
  </table>
...
function CariMhsw() {
  ...
  $lst->pageactive = "=PAGE=";
  $lst->pages = "<a href='?ndelox=$_SESSION[ndelox]&mhswpage==PAGE='>=PAGE=</a>";
  ...
  $lst->detailfmt = "<tr>
    ...
    <td class=cna=Keluar= width=100>
      <a href='?ndelox=$_SESSION[ndelox]&lungo=fnDropin&MhswID==MhswID='><img src='img/edit.png'>
      =MhswID=</a>
...
function fnProses() {
  ...
  if ($ProdiID == $mhsw['ProdiID'])
    die(PesanError('Error',
      ...
      <hr size=1 color=silver />
      <input type=button name='btnKembali' value='Kembali'
        onClick=\"location='?ndelox=$_SESSION[ndelox]&lungo=fnDropin&MhswID=$MhswID'\" />
...
function fnProsesnya() {
  ...
  $baru['StatusAwalID'] = 'D';
  $NIM = (AmbilOneField('prodi', 'ProdiID', $mhsw['ProdiID'], 'GunakanNIMSementara')=='Y')?  GetNextNIMSementara($TahunID, $baru) : GetNextNIM($TahunID, $baru);
