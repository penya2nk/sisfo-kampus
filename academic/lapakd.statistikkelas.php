...
include_once "../check_setting.php";
  ...
  $kelas_jmlsesi = GainVariabelx('kelas_jmlsesi', 6);
  $kelas_sesi = GainVariabelx('kelas_sesi', 1);
  TitleApps("Laporan Statistik Kelas");
  $tombols = '&nbsp;';
  ...
  if (empty($_SESSION['kelas_tahun'])) {
    ...
    if (empty($kelas)) $kelas = array();
      ...
      function fnCetakPerbandinganIPS(KelasID, jmlsesi) {
        ...
        lnk = "lapakd.statistikkelas.php?lungo=_CetakLaporanPerbandinganIPS&KelasID="+KelasID+"&jmlsesi="+jmlsesi+"&_rnd="+_rnd;
        win2 = window.open(lnk, "", "width=1000, height=800");
        if (win2.opener == null) childWindow.opener = self;
      }
	  function fnCetakPengambilanKHS(KelasID, sesi) {
        var _rnd = randomString();
        lnk = "lapakd.statistikkelas.php?lungo=_CetakDaftarPengambilanKHS&KelasID="+KelasID+"&sesi="+sesi+"&_rnd="+_rnd;
...
{	// Buat headernya dulu
  ...
  {	  $n++;
	  ...
	  $khs = AmbilFieldx('khs', "MhswID='$w[MhswID]' and Sesi='$sesi' and KodeID", KodeID, 'IPS, SKS'); 
...
function AmbilPerbandingan($kelasid, $tot, $p) {
  ...
  {	  $n++;
	  ...
	  $ips = AmbilOneField('khs', "MhswID='$w[MhswID]' and Sesi='1' and KodeID", KodeID, 'IPS'); 
	  $totarr[1] += $ips;
	  $p->Cell($leb1, $t, $ips, 1, 0, 'C');
	  for($i=2; $i<$tot; $i++)
	  {	  
		  $ipstemp = AmbilOneField('khs', "MhswID='$w[MhswID]' and Sesi='$i' and KodeID", KodeID, 'IPS'); 
		  $ipsdiff = $ipstemp-$ips;
	  ...
	  $ipk = AmbilOneField('khs', "MhswID='$w[MhswID]' and Sesi='1' and KodeID", KodeID, 'IP'); 
	  $p->Cell($leb1, $t, $ipk, 1, 0, 'C');
	  $totarr['ipk'] += $ipk;
	  $p->Ln($t);
  }
