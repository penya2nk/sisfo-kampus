<?php
error_reporting(0);
$lungo = (empty($_REQUEST['lungo']))? 'HalamanUtama' : $_REQUEST['lungo'];
$lungo();
...
include_once "../check_setting.php";
  ...
  $KelasID = GainVariabelx('KelasID');
  $kelas_tahun = GainVariabelx('kelas_tahun');
  $kelas_sesi = GainVariabelx('kelas_sesi', 1);
  $kelas_tahunajaran = GainVariabelx('kelas_tahunajaran', date('Y'));
  $kelas_prodimk = GainVariabelx('kelas_prodimk');
  TitleApps("Daftar dan Laporan Jurusan");
  $tombols = '&nbsp;';
  $wheretahun = "TahunID='$_SESSION[kelas_tahun]'";
	...
	{	$optkelas = AmbilCombo2('kelas', "Nama", 'Nama', $_SESSION['KelasID'], $wheretahun, 'KelasID');	
		...
		$optprodimk = AmbilCombo2('prodi', "concat(ProdiID, ' - ', Nama)", 'ProdiID', $_SESSION['kelas_prodimk'],
  ...
  if (empty($_SESSION['kelas_tahun'])) {
    ...
    if (empty($kelas)) $kelas = array();
	  ...
	  function fnCetakNilaiTengahSemester(KelasID, sesi, thnajaran) {
        ...
        lnk = "lapakd.statistikkelas2.php?lungo=_CetakNilaiTengahSemester&KelasID="+KelasID+"&sesi="+sesi+"&thnajaran="+thnajaran+"&_rnd="+_rnd;
        win2 = window.open(lnk, "", "width=1000, height=800, scrollbars");
        if (win2.opener == null) childWindow.opener = self;
      }
	  function fnCetakNilaiAkhirSemester(KelasID, sesi, thnajaran) {
        var _rnd = randomString();
        lnk = "lapakd.statistikkelas2.php?lungo=_CetakNilaiAkhirSemester&KelasID="+KelasID+"&sesi="+sesi+"&thnajaran="+thnajaran+"&_rnd="+_rnd;
        win2 = window.open(lnk, "", "width=1000, height=800, scrollbars");
        if (win2.opener == null) childWindow.opener = self;
      }
      </script>
ESD;
  ...
  $scripts2 = <<<ESD
	  ...
	  function fnCetakDistribusiMK(KelasID, prodimk) {
        var _rnd = randomString();
        lnk = "lapakd.statistikkelas2.php?lungo=_CetakDistribusiMK&KelasID="+KelasID+"&prodimk="+prodimk+"&_rnd="+_rnd;
        win2 = window.open(lnk, "", "width=1000, height=800, scrollbars");
        if (win2.opener == null) childWindow.opener = self;
      }
      </script>
...
{	// Buat headernya dulu
  ...
  $NamaKelas = AmbilOneField('kelas', 'KelasID', $kelasid, 'Nama');
  $p->SetFont('Helvetica', 'B', 10);
  ...
  $kota = AmbilOneField('identitas', 'Kode', KodeID, 'Kota');
  $t = 3;
  $p->Ln($t);
  ...
  $prodikelas = AmbilOneField('kelas', 'KelasID', $kelasid, 'ProdiID');
...
{	// Buat headernya dulu
  ...
  {	  $prodimk2 = (empty($prodimk))? AmbilOneField('kelas', 'KelasID', $kelasid, 'ProdiID') : $prodimk;
	  $p->Cell($lbrkls, $t, $prodimk2, 1, 0,'C');
	  $arrKelas[] = AmbilOneField('kelas', 'KelasID', $kelasid, 'Nama');;
  }	
  ...
  {	  $n++;
	  ...
	  foreach($arrKelas as $kelas)
	  {	$cari = AmbilOneField('jadwal', "NamaKelas='$kelas' and Nama='$w[Nama]' and KodeID", KodeID, 'JadwalID');
