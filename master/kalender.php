...
// *** Main ***
TitleApps("Kalender Tahunan");

$lungo = (empty($_REQUEST['lungo']))? 'Kalender' : $_REQUEST['lungo'];
...
{	$arrTahun = GetTahunArray($tahun-5, $tahun+5);
	...
	echo "<td class=menuitem width=25 align=center><a href='?ndelox=$_SESSION[ndelox]&lungo=&tahunkalender=".($tahun-10)."&bulankalender=$bulan'><<</a></td>";
	foreach ($arrTahun as $a) {
		$sel = ($tahun == $a)? 'class=menuaktif' : 'class=menuitem';
		echo "<td $sel width=60 align=center><a href='?ndelox=$_SESSION[ndelox]&lungo=&tahunkalender=$a&bulankalender=$bulan'>$a</a></td>";
	}
	echo "<td class=menuitem width=25 align=center><a href='?ndelox=$_SESSION[ndelox]&lungo=&tahunkalender=".($tahun+10)."&bulankalender=$bulan'>>></a></td>";
	echo "</table>";
	...
	foreach ($arrBulan as $a) {
		...
		echo "<td $sel width=60 align=center><a href='?ndelox=$_SESSION[ndelox]&lungo=&tahunkalender=$tahun&bulankalender=$a'>".UbahKeBulanIndonesia($a)."</a></td>";
	}
	echo "</table>
	</div>
</div>
	...
	echo "<td><table id='example' class='table table-sm table-striped' style='width:90%' align='center'>
